...
function ClearParamValues() {
    ...
    if (document.title == "Iview" || document.title == "Listview") {
        ...
        try {
            ...
            $("select.trySelect, select.trySelectPl, select.trySelectMs").each(function (ind, ele) {
                try {
                    $(ele).select2('destroy');
                } catch (ex) {}
            });
...
function CallSearchOpen(plEle = "") {
    ...
    if (isPlDepParBound) {
        try {
            ...
            var plAdvanceSearchUrl = `./iviewAutoComplete.aspx?isIV=${!isListView}&srchTxt=${""}&fldname=${plEle.id}&transid=${iName}&key=${$("#hdnKey").val()}&params=${$("#hdnparamValues").val()}&dF=${thisDfVal}&isSelect=${thisPType}`;
