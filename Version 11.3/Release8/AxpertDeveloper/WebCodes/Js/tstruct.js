                ...
                if (staticRunMode) {
                    ...
                    if (autoArrangeFillGap) {
                        _.map($(gsElem).data('gridstack').grid.nodes, function (el) {
                            ...
                            var newNode = { ...el.data('_gridstack_node') };
                            var x = 0;
                            var y = node.y;
                            //to move y
        ...
        if (theMode == "design") {
            ...
            $("table" + this + " th, " + this + " table th").resizable({
                ...
                resize: function (event, ui) {
                    ...
                    $(this).css({ "width": ui.size.width, "min-width": ui.size.width });
...
function clearHiddenGridstack(divId, isTable = false) {
    ...
    /*var hiddenGS =*/
    try {
        compressedMode = designObj[0].compressedMode;
        if (!isMobile) {
        ...
        if (!isTable) {
            ...
            currentDC.fieldsDesign.filter(function (elm, ind) {
                ...
                try {
                    ...
                    var tempRowObj = currentDC.fieldsDesign.filter(function (elmns) { return elmns.y >= elm.y && elmns.y <= (elm.y + elm.height - 1) });
                ...
                if (theMode == "render" && !staticRunMode) {
                    ...
                    fullFieldHeight = { "height": (gsiPX + gsConf[compressNormalMode].labelHeight) + "px" };
                    //debugger;
                ...
                if (!elm.visibility) {
                    ...
                    if (!$("#" + divId).data('gridstack')) {
                        if (currentDC.isGrid == "T") {
                            ...
                            var dvElem = $("#dvGrid" + elm.fld_id);
                            dvElem.attr({ "data-gs-x": elm.x, "data-gs-y": elm.y, "data-gs-width": elm.width, "data-gs-height": elm.height });
                            dvElem.find(".form-control").css(gsiHeight);
                            applyGridStackCalcHeight(dvElem.find("textarea.gridstackCalc, div.divImgBorder, input[type=button].hotbtn.btn"), elm, gsiHeight, fullFieldHeight);
...
function goToDesignMode() {
    ...
    var tstCustHTML = $("#breadcrumb-panel").length != 0 ? $("#breadcrumb-panel").get(0).outerHTML : "";
...
function DesignFormDcLayouts() {
    ...
    displayBootstrapModalDialog("DC Layouts", "md", "200px", "", dvHtml);
}
...
function AddFormLabel(ele=""){
    ...
    displayBootstrapModalDialog("Caption", "sm", "50px", "", dvHtml,false, ()=> {$(".modal-content").append(footerHTML);});
}
...
function LabelHyperlink(ele,ctType)
{
  ...
  dvHtml +=`</tbody></table></div>`; 
    ...
    <button id="btnlcDel" title="Delete Hyperlink" value="Delete" class="coldbtn btn pull-right hide" onclick="callBackHyperlinkDelete('${ele}','${ctType}')">Delete</button>
    </div>`;    
    dvHtml += `</div></div>`;
...
function callBackHyperlinkDelete(ele, ctType) {    
    let flJson = JSON.parse(ctType == "label" ? formLabelJSON : buttonFieldFontJSON);
    $.each(flJson, (key, value) => {
        if (flJson[key].id == ele) {
            flJson[key].hyperlinkJson = "";
            return;
        }
    });
    ctType == "label" ? formLabelJSON = JSON.stringify(flJson) : buttonFieldFontJSON=JSON.stringify(flJson);
    SavePublishDesignerJSONWeb(false);
}
    ...
    if(selectedStName!="" && StrType!="page"){
        try {
            $.ajax({
                ...
                async: true,
                data: JSON.stringify({
                    StrType:StrType,StructName:selectedStName
                }),
                dataType: 'json',
                    ...
                    if (data.d != "") {
                        ...
                        if (jVal != "" && jVal.result.row!="")
                        {
                            ...
                            $.each(jVal.result.row, function (ind, obj){ 
                                if (obj?.name != "" || obj?.pname != "") {
                                    output.push(`<option value="${obj?.pname || obj?.name}">${obj?.pname || obj?.caption}</option>`);
                                }
                            });
                            $(".designTableHyperlink tbody tr").each(function(ind,val){
...
function toggleCompressedMode(type) {
    ...
    if ($("#ckbCompressedMode").prop("checked") == true) {
        ...
        if ($("link[href='../Css/compressedTstruct.min.css?v=36']").length > 0)
            $('link[rel=stylesheet][href~="../Css/compressedTstruct.min.css?v=36"]').remove();
    } else {
        //checkbox is checked
        ...
        if ($("link[href='../Css/compressedTstruct.min.css?v=36']").length != 1)
            $('head').append($('<link rel="stylesheet" type="text/css" />').attr('href', '../Css/compressedTstruct.min.css?v=36'));
    }
...
function applyGridStackCalcHeight(currentElm, elm, gsiHeight, fullFieldHeight) {
    if (theMode != "design") {
        ...
        if (typeof CKEDITOR.instances[$(currentElm).attr("id")] != "undefined") {
            ...
            CKEDITOR.instances[$(currentElm).attr("id")].on("instanceReady", function () { this.resize('100%', gsiPX) });
        } else {
    ...
    if (isFormChange) {
        ...
        try {
            $.ajax({
                ...
                async: true,
                data: JSON.stringify({
                    Transid: transid, DesignType: Savetype, Content: dsnJSON, SavedId: ''
                }),
                dataType: 'json',
                contentType: "application/json",
                success: function (data) {
...
function PublishDesignerJSONWeb() {
    ...
    if (isFormChange) {
        ...
        $.ajax({
            ...
            async: true,
            data: JSON.stringify({
                Transid: transid, DesignType: Savetype, Content: '', SavedId: SavedId
            }),
            dataType: 'json',
            contentType: "application/json",
            success: function (data) {
...
function setSelectedDesignElement(elem) {
    ...
    $(".ui-resizable").find(".thhead").css({ "border": "", "border-right": "" });
...
function OnTstructLoad() {
    ...
    if ((typeof LoadResult) == "string") {
        ...
        try {
            ...
            if (theMode != "design") {
                ...
                if (recordid == "0" && (axInlineGridEdit || (!axInlineGridEdit && AxpGridForm == "form"))) {
                    DCFrameNo.forEach(function (dcID) {
                        if (DCIsGrid[dcID - 1] == "True") {
    ...
    $('.lblcleargrid').html(appGlobalVarsObject.lcm[302]);
    $('.gridAddRow').html(appGlobalVarsObject.lcm[477]);
    $('.lblExportGrid').html(appGlobalVarsObject.lcm[486]).attr("title", appGlobalVarsObject.lcm[486]);
...
function MainFocus(fldObj) {
    ...
    if (fldId != undefined) {
        ...
        //else
        //UnHighlightRow(); // != "FocusEvent"//event != "FocusEvent"
        ...
        try {
            if (transid == "ad__d") {
                if (fldId == "tablename000F1") {
                    let existValue = GetFieldValue(fldId);
                    dctableOnChange = existValue != "" ? existValue : dctableOnChange;
                }
            }

            if (fldId == "datawidth000F1") {
                let dwValue = GetFieldValue(fldId);
                fldDataWidthChange = dwValue != "" ? dwValue : fldDataWidthChange;
            }
        } catch (ex) { }
...
function MainBlur(fldObj) {
    ...
    if (transid == "axcad") {
        if (fldId == "cardtype000F1") {
            cardFieldsShowHideLogic();
        }
        if (fldId == "cardbgclr000F1") {
            cardBackgroundPickerLogic();
        }
    }

    if (transid == "ad_s" && fieldID == 'control_type000F1' && newFldValue == "T") {
        $("#exp_editor_fcscript000F1").parent().find('.CodeMirror .CodeMirror-sizer').css("margin-left", "30px");
        $("#exp_editor_fcscript000F1").parent().find('.CodeMirror .CodeMirror-linenumbers').css("width", "29px");
    }
        ...
        if (value != "") {
            switch (dwbfldtype) {
                case 'Email':
                    ...
                    break;
                case 'URL':
                    vregex = "^((https?):\/\/)?([w|W]{3}\.)+[a-zA-Z0-9\-\.]{3,}\.[a-zA-Z]{2,}(\.[a-zA-Z]{2,})?$";
                    if (!CheckIsvalidfield(value, vregex)) {
                        var cutMsg = eval(callParent('lcm[1044]'));
                        patternErrMsg = (tmpFldCaption == "" ? tmpFldCaption : tmpFldCaption + ": ") + cutMsg;
                    }
                    break;
                case 'IP Address':
                    ...
                    break;
                case 'Password':
                    vregex = "^(?=(.*[a-z]){3,})(?=(.*[A-Z]){2,})(?=(.*[0-9]){2,})(?=(.*[!@#$%^&*()\-__+.]){1,}).{8,}$";
                    if (!CheckIsvalidfield(value, vregex)) {
...
function DoSetDependents(fieldID) {
    ...
    if (transid == "mntss" && fieldID == "sql_editor_test000F2" && recordid != "0") {
        sql_editor_depflds = GetFieldValue(GetComponentName("tablename", GetDcNo("tablename")));
        sql_editor_depflds += "," + GetFieldValue(GetComponentName("ptableid", GetDcNo("ptableid")));
        sql_editor_depflds += "," + GetFieldValue(GetComponentName("firstcolvalue", GetDcNo("firstcolvalue")));
        sql_editor_depflds += "," + GetFieldValue(GetComponentName("sourcefield", GetDcNo("sourcefield")));
        sql_editor_depflds += "," + GetFieldValue(GetComponentName("sourcetable", GetDcNo("sourcetable")));
        sql_editor_depflds += "," + GetFieldValue(GetComponentName("dynamicfiltercnt", GetDcNo("dynamicfiltercnt")));
    }
    ...
    if (transid == "ad__d" && fieldID == "tablename000F1") {
        DcTableExecuteSql(fieldID);
    }

    if (transid == "mntss" && fieldID == "savenormalized000F2") {
        fldSNExecuteSql(fieldID);
    }

    if (fieldID == "datawidth000F1") {
        fldWidthChange(fieldID);
    }
}
...
function SuccChoicesgetDep(result, eventArgs) {
    if (result.toLowerCase().indexOf("access violation") === -1) {
        ...
        try {
            if (AxActDepFld == "selectflddl000F2") {
                if (callParentNew("transid") == "mntss" && callParentNew("actionfldflag") == "change" && GetFieldValue(GetComponentName("modeofentry", GetDcNo("modeofentry"))) == "Select From Form" && $("#mastertstructddl000F2").val() != "" && $("#masterflddl000F2").val() != "" && $("#savenormalized_form000F2").val() == "T") {
                    customExecuteSql();
                }
            }
        } catch (ex) { }
        ...
        try {
            if (callBackFunDtls == "") {
                if (isPKItemSelected == true || $j("#" + AxActDepFld).hasClass('fldAutocomplete') || $j("#" + AxActDepFld).hasClass('fldmultiSelect') || (blurNextPreventId != "" && $("#" + blurNextPreventId).is(".editLayoutFooter button") || (!$.isEmptyObject(blurNextPreventElement) && $(blurNextPreventElement).is(".editLayoutFooter button")))) {
...
function CheckGrdisplayHead(tabNo, fieldName, frRowno, status) {
    ...
    if (dispCaption.length == dispVal.length) {
        ...
        if ($j("#" + disHeaddiv))
            $j("#" + disHeaddiv).css({ "position": "relative" }).html(ntxt);
...
function TstructLabelhyperlink(obj) {
    ...
    else if(hlParamData.type=="iview")
    {
        ...
        if(hlParamData.ivtstpopup=="true"){
            ...
            var loadpopup = 'ivtoivload.aspx?ivname=' + hlParamData.structName + "&" + params, popname;
            createPopup(loadpopup);
        }else{
    ...
    if (status == 0) {
        ...
        if (!$("#divDc" + CurrTabNo).is(":visible") && typeof TabDCAlignmentStatus[CurrTabNo - 1] == "undefined") {
            ...
            $(document).on("click", "#myTab li", function (e) {
                ...
                $(".tab-content").find($(this).find("a").attr("href")).addClass("active in");
            });
        }
        ...
        if (AxFormContFldSetFocus[j] != "") {
            ...
            if (strFld[2] == "setfocus") {
                if (!IsDcGrid(dcNo)) {
                    ...
                    if (isMobile && $("#" + fieldId).hasClass("fldAutocomplete"))
                        continue;
                    else
                        $j("#" + fieldId).focus();
                }
...
function AssignJQueryEvents(dcArray) {
    ...
    for (var i = 0; i < dcArray.length; i++) {
        ...
        if ($j(dvId).length > 0) {
            ...
            $j(dvId).find('.dropdown-main').remove();
            $j(dvId).find('.dropdown-display-label').remove()

            var jsonData = JSON.parse("{\"data\":[]}");
            $j(dvId).find('.dropdown-mul').dropdown({
                data: jsonData.data,
                //limitCount: 40,
                multipleMode: 'label',
                choice: function () {
            ...
            if (multiSelectLoadVals.length > 0) {
                $.each(multiSelectLoadVals, function (index, elem) {
                    var msFldname = elem.split("♦")[0];
                    var msFldsVals = elem.split("♦")[1];
                    var fieldNameMs = msFldname.substring(0, msFldname.lastIndexOf("F") - 3);
                    var dvRow = "";
                    if (axInlineGridEdit == false) {
                        if (typeof $j(dvId).find(".editWrapTr").attr("id") != "undefined")
                            dvRow = $j(dvId).find(".editWrapTr").attr("id").substring($j(dvId).find(".editWrapTr").attr("id").lastIndexOf("F"), $j(dvId).find(".editWrapTr").attr("id").lastIndexOf("F") - 3);
                        else
                            dvRow = $j(dvId).attr("id").substring($j(dvId).attr("id").lastIndexOf("F"), $j(dvId).attr("id").lastIndexOf("F") - 3);
                    }
                    else
                        dvRow = $j(dvId).attr("id").substring($j(dvId).attr("id").lastIndexOf("F"), $j(dvId).attr("id").lastIndexOf("F") - 3);
                    var fldmsRow = msFldname.substring(msFldname.lastIndexOf("F"), msFldname.lastIndexOf("F") - 3);
                    if (IsGridField(fieldNameMs) && dvRow == fldmsRow) {
                        var dropdownlsHmtl = "";
                        let msSep = $("#" + msFldname).attr("data-sep");
                        $.each(msFldsVals.split(msSep), function (index, el) {
                            dropdownlsHmtl += "<span class=\"dropdown-selected\">" + el + "<i class=\"del\" data-id=\"" + el + "\"></i></span>";
                        });
                        $("#" + msFldname).next(".dropdown-display-label").find(".dropdown-chose-list").prepend(dropdownlsHmtl);
                        $("#" + msFldname).parents(".dropdown-mul").click();
                    }
                });
            }
            $j(dvId).find("input,select,textarea:not(#txtCommentWF)").unbind("focus");
            $j(dvId).find("input:not([class=AxAddRows],[class=AxSearchField]),select:not(#selectbox),textarea:not(#txtCommentWF):not(.labelInp)").focus(function () {
                MainFocus($j(this));
            });
            ...
            $j(dvId).find("textarea:not(#txtCommentWF):not(.labelInp),[id]:text:not([class=AxAddRows],[class=AxSearchField]),:password").blur(function () {
                MainBlur($j(this));
                if (axInlineGridEdit) {
                    var fldObj = $j(this);
                    var fldId = fldObj.attr("id");
            ...
            $j(dvId).find(":checkbox:not([class=chkAllList]):not(.tokenSelectAll),:radio").not(".chkShwSel").change(function () {
                MainBlur($j(this));
            });
            ...
            $j(dvId + " .pickimg").unbind("click");
            //function on click of picklist image
            $j(dvId + " .pickimg").click(function () {
                DisplayPickList($j(this));
            });

            $j(dvId + " .picklisttxtclear").hover(function (e) {
            ...
            $j(dvId + " .picklisttxtclear").unbind("click");
            $j(dvId + " .picklisttxtclear").click(function (e) {
            ...
            $j(dvId + " .autoinputtxtclear").unbind("click");
            $j(dvId + " .autoinputtxtclear").click(function (e) {
        ...
        $j(".gridoptions").on("click.gridoptionsa", function (e) {
            ...
            if (typeof $(this).closest("a") != "undefined" && $(this).closest("a").length > 0) {
                ...
                $("#divDc" + dId + " .grdButtons").css("left", $("#" + $(this).closest("a").attr("id")).offset().left - 20);
            }
            else if (typeof $(this).closest("div").attr("id") != "undefined" && !isWizardTstruct) {
                dId = $(this).closest("div").attr("id").substring(4);
                //  $("#divDc" + dId + " .grdButtons").css("left", $("#" + $(this).closest("div").attr("id")).offset().left - 20);
                ...
                $("#divDc" + dId + " .grdButtons").css("left", $(this).parent().next("div").offset().left + 20);
            }
        ...
        $("#heightframe").off('keydown.heightframeb');
        $("#heightframe").on("keydown.heightframeb", function (event) {
        ...
        $j(document).on("click", "body", function (e) {
            if (e.target.className != "gridoptions glyphicon glyphicon-chevron-down" && $(".gridoptions").length > 0 && !isMobile) {
                let dId;
        ...
        $j(dvId).find('.dropdown-mul').off('click.dropdown-mula');
        $j(dvId).find('.dropdown-mul').on('click.dropdown-mula', function (event) {
        ...
        $j(dvId).find(".glyphicon-th").off('click.glyphicon-tha');
        $j(dvId).find(".glyphicon-th").on('click.glyphicon-tha', function (event) {
...
function CallAddRowPerfWS(dcStr, rowNo) {
    ...
    var ixml = '<sqlresultset axpapp="' + proj + '" transid="' + transid + '" recordid="' + rid + '" dcname="' + dcStr + '" rowno="' + dbRowNo + '" dcnames="' + visDcname + '" sessionid="' + sid + '" trace="' + filename + '" >';
    try {
        ASB.WebService.AddRowPerfWS(ChangedFields, ChangedFieldDbRowNo, ChangedFieldValues, DeletedDCRows, ixml, dcStr, tstDataId, SuccessCalAddData, ErrorAddRowWS);
    } catch (exp) {
        AxWaitCursor(false);
        ShowDimmer(false);
        showAlertDialog("error", ServerErrMsg);
    ...
    for (var i = 0; i < fields.length; i++) {
        ...
        if (fld.length > 0) {
            ...
            if (IsMasterRow == true) {
                ...
                if (fld.hasClass("fldAutocomplete") || fld.hasClass("fldmultiSelect")) {
                    $j(fldName).val("");
                }
            } else {
                if (fldType == "select-one")
    ...
    for (var i = 0; i < FNames.length; i++) {
        ...
        if (allowEmpty.toLowerCase() == "f" && visible.toLowerCase() == "f") {
            ...
            if (isGrid == true) {
                ...
                for (var k = 1; k < (axInlineGridEdit ? rCount + 1 : rCount); k++) {
                    ...
                    if (IsAxFldEmpty(i, k, dcNo) == false) {
                        return false;
                    }
                }
            } else {
                if (IsAxFldEmpty(i, "000", dcNo) == false)
                    return false;
            }
        }
        ...
        if (FNames[i].toString().toLowerCase().indexOf('validate_') > -1 && FDataType[i].toString().toLowerCase() == "image") {
            ...
            if (FNames[i].indexOf('validate_gridattach_') > -1) {
                ...
                if (controlName[0].src == '' || controlName[0].src == null || controlName[0].src.indexOf('/AxpImages/upload.png') > -1) {
...
function IsAxFldEmpty(idx, rowNo, dcNo, calledFrom) {
            ...
            if (resJson != undefined) {
                ...
                if (result != "") {
                    ...
                    if (restxt != ErrStr) {
                        ...
                        if (recidIdx != -1) {
                            ...
                            if (presentFrame.attr("id") === "homePageDynamicFrame") {
                                if (parent.reloadTheWidget)
                                    parent.reloadTheWidget({ widgetTarget: presentFrame.data("calledElemntId") });
                            }
                            ...
                            if (callParentNew("isDWB") && (transid == "sect" || transid == "ad_ih")) {
                                try {
                        ...
                        if (recordidax && transid == "axcad") {
                            clearCardCache(recordidax);
                        }
                    } else {
...
function GetFormLoadData(tstQureystr, isDraft, forceRefresh = "false") {
    ...
    try {
        $.ajax({
            ...
            async: true,
            data: JSON.stringify({
                key: tstDataId, tstQureystr: tstQureystr, isDraft: isDraft, forceRefresh: forceRefresh
            }),
            dataType: 'json',
            contentType: "application/json",
            success: function (data) {
                ...
                if (result.toLowerCase().indexOf("access violation") === -1) {
                    ...
                    if (result != "") {
                        ...
                        $("#hdnTabHtml").val(resval[2]);
                        var tstPerfVars = resval[3];
                        if (tstPerfVars != "") {
                        ...
                        isTstPostBackVal = resval[0] + "*$*" + resval[1];
                        //}
                    }
                } else {
                    blurNextPreventId = "";
...
function GetLoadData(recid, tstQureystr) {
    try {
        $.ajax({
            ...
            async: true,
            data: JSON.stringify({
                key: tstDataId, recordid: recid, tstQureystr: tstQureystr
            }),
            dataType: 'json',
            contentType: "application/json",
            success: function (data) {
...
function SucceededCallbackDelTst(result, eventArgs) {
    ...
    for (var ind = 0; ind < resval.length; ind++) {
        ...
        if (myJSONObject.error) {
            ...
            if (recordid && transid == "axcad") {
                clearCardCache(recordid, true);
            }

            if (window.frameElement && window.frameElement.id && window.frameElement.id === "popupIframeRemodal") {
                if (callParentNew("isDWB") && transid == "ad_ih") {
                    try {
                        dwbCustomDeleteRedirect();
                    } catch (ex) { }
                }
                eval(callParent('isSuccessAlertInPopUp') + "= true");
                SetFormDirty(false);
...
function HideTaskList(btnClicked) {
    ...
    if (transid == "paget" && AxActiveAction == "") {
        var recIdVal = "", tstid = "";
        if (window.location.search.split('?').length > 1) {
            var params = window.location.search.split('?')[1].split('&');
            for (var i = 0; i < params.length; i++) {
                var key = params[i].split('=')[0];
                if (key == "oldrecid" || key == "oldferecid") {
                    recIdVal = params[i].split('=')[1];
                }
                else if (key == "structname") {
                    tstid = params[i].split('=')[1];
                }

            }
        }
        if (tstid != "")
            window.location.href = "tstruct.aspx?act=load&transid=" + tstid + "&recordid=" + recIdVal + "&AxSplit=true";
    }
}
...
function SearchOpen(txtobj) {
    ...
    if ($j("#" + fname1).hasClass("fldAutocomplete")) {
...
function SetSelectedValue(listcontrol, listctrl) {
    ...
    if (listcontrol[listcontrol.selectedIndex].value != -1) {
        ...
        UpdateParentArray();
        try {
            if (parentFillField && !parentFillField.disabled)
                parentFillField.focus();
        } catch (exp) { }
        if (window.opener == null)
...
function AssignHTML(result, calledFrom, source) {
    ...
    for (var i = 0; i < resArr.length; i++) {
        ...
        if (source == "GetTabData" && isWizardTstruct && wizardObj.tabCallback) {
            wizardObj.successRequestFreshTabData("DivFrame" + dcNo);
...
function ShowAxpFileAttachPopUp(obj) {
    ...
    if (transid == "axcad" && attFldName == "axpfile_imgcard") {
        window["ax_changeValidateFileExtensionParams"] = (filename, fileType, CustomList) => {
    ...
    function CallbackFunctionBootstrapAxpFile() {
        ...
        if (transid == "axcad" && attFldName == "axpfile_imgcard") {
            delete window["ax_changeValidateFileExtensionParams"];
        }
    }
}
...
function ConstructGridAttachHTML(fldName, fNo, fldValue, hdnValue, rowFrmNo) {
    ...
    for (var i = 0; i < arrFileNames.length; i++)
        if (arrFileNames[i] != undefined || arrFileNames[i] != "") {
            ...
            fileLinks += "<div id=\"Link_" + rowFrmNo + "_" + finalFileName + "\"><a onclick=\"DeleteFileFromRow('" + fldName + "','" + rowFrmNo + "','" + arrFileNames[i] + "')\"><i class=\"glyphicon glyphicon-remove close icon-arrows-remove attachmentcrossicon\"></i></a><a href=\"javascript:void(0)\"  tabindex=\"-1\" class='grdAttach handCur' onclick='ShowGridAttLink(\"" + filePath + arrFileNames[i] + "\")'>" + arrFileNames[i] + "</a></div>";
        }
    ...
    for (var i = 0; i < arrFileNames.length; i++) {
        ...
        if (calledfrom == undefined) {
            if (rowNo == "000" && fldValue != "") {
                ...
                if ($(".attach-files").parent("#GridAttach" + rowFrmNo).find(".attach-files").height() > 20) {
            ...
            if ($("#gridHd" + dcNo + " tbody td .attach-files").parent("#GridAttach" + rowFrmNo).find(".attach-files").height() > 20) {
    ...
    for (var i = 0; i < arrFileNames.length; i++) {
        ...
        if (calledfrom == undefined) {
            if (rowNo == "000" && fldValue != "") {
                ...
                if ($(".attach-files").parent("#GridAxpFile" + fldName).find(".attach-files").height() > 20) {
            ...
            if ($("#gridHd" + dcNo + " tbody td .attach-files").parent("#GridAxpFile" + fldName).find(".attach-files").height() > 20) {
...
function SetGridRefWithdcImage(fldName, fldValue) {
    ...
    if (fldValue.indexOf(',') > -1) {
        ...
        for (var i = 0; i < arrFileNames.length; i++) {
            fileLinks += "<div id=\"Link" + arrFileNames[i] + "\"><a href='javascript:void(0)' id='grdAtt_hlnk_001F2' tabindex='-1' class='grdAttach handCur' onclick='ShowGridAttLink(\"" + filePath + referRecId + "-" + arrFileNames[i] + "\")'>" + arrFileNames[i] + "</a></div>";
...
function clickOnWrkBtn() {
    ...
    var optionSel = $("#selectbox").find('li.selected').text();
...
function SlectBoxWrf(wftype) {
    ...
    $('#consumergoods2').modal('show');
    $("#comment").val(wftype);
    $("#commentimp").attr('style', 'display: block,margin-left: 75px,margin-top: -22px;');
    $("#comment").removeClass("hide");
    $("#btnWrfSave").removeClass("hide");
    $("#btnWrfCancel").removeClass("hide");
    $("#lblreject").text("");
    if ($("#comment").length) $("#comment").focus();

    $("#consumergoods2").on('hidden.bs.modal', function () {
        resetActions();
    });
}
...
/**
 * set wizard/default tstructmode
 * @author Prashik
 * @Date   2018-05-30T11:08:25+0530
 * @return {undefined}
 */
function switchTstructMode() {
    ...
    if (typeof isWizardTstruct != "undefined" && isWizardTstruct) {
        ...
        $("#wizardTstructWrapper").remove();
    }
}

function createTstructWizard() {
    $("#dvFooter").hide();
    $(".dcTitle").hide().next("hr.hrline").hide();
    try {
        if (typeof designObj != "undefined" && designObj[0] != null && designObj[0].selectedLayout != null && designObj[0].selectedLayout != "" && designObj[0].selectedLayout == "tile") {
            $(".dcTitle").show().next("hr.hrline").show();
        }
    } catch (ex) { }

    $("#tstIcons #imgSaveTst,#tstIcons [title='Save' i]").hide();
    ...
    $("#wizardTstructWrapper").append(Array.prototype.reverse.call($("#pagebdy").prevAll()).detach());

    $("#searchBar").addClass("oldSearchBar");
    $("#wizardTstructWrapper").append('<div style="" id="wizardWrappper">' +
        '<div class="wizardContent">' +
        '<div id="wizardHeader">' +
        '</div>' +
        '<div id="toolBarBtns"></div>' +
        '<div id="wizardBodyFooterWrapper">' +

        '<div id="wizardBodyContent" class="wizardBodyContent">' +
        '<div id="wizardFormContainer"></div>' +
        '</div>' +


        '<div class="wizardFooter">' +
        '<div class="wizardNextPrevWrapper">' +

        '<button onclick="wizardObj.checkClick(this,\'prev\')" type="button" id="wizardPrevbtn" class="wizardPrevNxtBtns waves-effect btn themeButton cutsomColorbtn left" title="Previous"><span class="material-icons">skip_previous</span> <span class="wizard-next">Prev</span></button>' +

        '<button onclick="javascript:NewTstruct();" type="button" id="wizardCancelbtn" class="wizardPrevNxtBtns waves-effect btn coldbtn left" title="Cancel"><span class="material-icons">close</span> <span class="wizard-next">Cancel</span></button>' +
    ...
    $("#searchBar").clone().removeClass('oldSearchBar').appendTo("#toolBarBtns"); //cloning for events
    $("#searchBar.oldSearchBar").remove();
    let wizardType = AxwizardType === "modern" ? "flat" : AxwizardType;
    wizardObj = new WizardComp({
        progress_bar: wizardType
    });
    wizardObj.tabCallback = false;
    var dcIDS = DCFrameNo.map(function (fn) {
        return "DivFrame" + fn
    });
    ...
    wizardObj.checkNxtPrevBtns = function (setFocus) {
        if (setFocus == undefined) setFocus = true;
        var totalSteps = $("#wizardHeader .step").filter(":visible").length;
        var curStep = $("#wizardHeader .step").filter(":visible").index($("#wizardHeader .step.active"));
        var curDcNo = GetWizardDcNo($j("#wizardWrappper #wizardHeader .step.active").data("target"));
        ...
        if (curStep == 0) {
            ...
            $("#wizardNextbtn").show().attr({
                ...
                "title": "Next"
            }).html("<span class='material-icons'>skip_next</span> <span class='wizard-next'>Next</span>");
        } else if (curStep == (totalSteps - 1)) {
            $("#wizardPrevbtn").show();
            if (transid == "ad_lg") {
                $("#wizardNextbtn").show().attr({
                    "onclick": "javascript:btn24onclick();",
                    "title": "Publish to runtime"
                }).html("<span class='material-icons'>sync_alt</span> <span class='wizard-next'>Publish to runtime</span>");
            } else {
                $("#wizardNextbtn").show().attr({
                    "onclick": wizardSaveActionBtn,
                    "title": "Save"
                }).html("<span class='material-icons'>save</span> <span class='wizard-next'>Save</span>");
            }
        } else {
            $("#wizardPrevbtn").show();
            $("#wizardNextbtn").show().attr({
    ...
    wizardObj.completeCurrentStep = function () {
        var presentStep = $("#wizardHeader .step.in-progress");
        var nextStep = presentStep.nextAll(":visible").first();
        presentStep.removeClass('in-progress active').addClass('complete');
        nextStep.addClass('in-progress active');
        wizardObj.showActiveData(nextStep.data('target'));
    }

    wizardObj.showActiveData = function (target) {
        var thisDC = target.startsWith("divDc") ? parseInt(target.substr(5), 10) : target.startsWith("sp") ? parseInt(target.substr(target.lastIndexOf("F") + 1), 10) : target.startsWith("#gridHd") ? parseInt(target.substr(7), 10) : target.startsWith("#tab-") ? parseInt(target.substr(5), 10) : target.startsWith("DivFrame") ? parseInt(target.substr(8), 10) : 0;
        if ($("#wizardBodyContent").children().not('#wizardFormContainer').length > 0) {
            $("#wBdr").append($("#wizardBodyContent").children().not('#wizardFormContainer').detach());
        }
        if ($("#" + target).length) {
            if (!isMobile) {
                var curFrNum = target.slice(8);
                if ($("#myTab").find("#li" + curFrNum).find(".gridoptions").length > 0) {
                    $("#wizardBodyContent").append('<div class="gridoptionlabel" id="grdOptId' + curFrNum + '"><span>Grid Options </span></div>');
                    $("#grdOptId" + curFrNum).append($("#myTab").find("#li" + curFrNum).find(".gridoptions").detach());
                }
                else if ($("#head" + curFrNum).find(".gridoptions").length > 0) {
                    $("#wizardBodyContent").append('<div class="gridoptionlabel" id="grdOptId' + curFrNum + '"><span>Grid Options </span></div>');
                    $("#grdOptId" + curFrNum).append($("#head" + curFrNum).find(".gridoptions").detach());
                }
                else if ($("#grdOptId" + curFrNum).length > 0) {
                    $("#wizardBodyContent").append($("#grdOptId" + curFrNum).detach());
                }

            }
            else if (isMobile) {
                $(".btnMobile").addClass("hide");
                if ($("#divDc" + thisDC).find(".mobilewrapperForEditFields").length > 0)
                    $("#wrapperForEditFields" + thisDC + " .grdButtons.btnMobile").removeClass("hide");
            }
            $("#wizardBodyContent").append($("#" + target).detach());

            if (IsDcGrid(thisDC) && !axInlineGridEdit) {
                let rowId = $("#wrapperForEditFields" + thisDC).find(".editWrapTr").attr("id");
                let rowLstNo = rowId.substring(rowId.lastIndexOf("F"), rowId.lastIndexOf("F") - 3);
                var fields = GetGridFields(thisDC);
                CallEvaluateOnAddPerf(thisDC, rowLstNo, fields, "AddRow");
                UpdateFieldArray(ConstructFieldName(axpIsRowValid + thisDC, thisDC, parseInt(rowLstNo, 10)), rowLstNo, "", "parent", "AddRow");
            }

            tabFocusEventHandler('unbind', "pagebdy");
            tabFocusEventHandler('bind', "wizardWrappper", true, true);
            if (isMobile && $(".wrapperForGridData" + (thisDC) + " table tbody tr").length > 0) {
                $("#colScroll" + (thisDC)).show();
            }
            try {
                AxAfterLoadTab(thisDC);
            }
            catch (ex) { }
        } else if (isWizardTstruct) {
            //divId = divId.startsWith("#") ? divId.substr(1) : divId;
            GetTabData(thisDC.toString());
            wizardObj.tabCallback = true;
        }
    }

    wizardObj.successRequestFreshTabData = function (target) {
        var thisDC = target.startsWith("divDc") ? parseInt(target.substr(5), 10) : target.startsWith("sp") ? parseInt(target.substr(target.lastIndexOf("F") + 1), 10) : target.startsWith("#gridHd") ? parseInt(target.substr(7), 10) : target.startsWith("#tab-") ? parseInt(target.substr(5), 10) : target.startsWith("DivFrame") ? parseInt(target.substr(8), 10) : 0;
        //if ($("#wizardBodyContent").children().not('#wizardFormContainer').length > 0) {
        //    $("#wBdr").append($("#wizardBodyContent").children().not('#wizardFormContainer').detach());
        //}
        ...
        wizardObj.checkNxtPrevBtns();
        if (recordid > 0) {
            $("div#wizardWrappper div#wizardHeader div.step").addClass("complete");
        }
    ...
    dcIDS.forEach(function (dcID) {
        if (dcID.substr(dcID.length - 1) != "1") {
            return;
        }
    });

    wizardObj.validateTheKey = function () {
        var curDcNo = GetWizardDcNo($j("#wizardWrappper #wizardHeader .step.active").data("target"));
        if (curDcNo != undefined && curDcNo != "" && !ValidateBeforeSubmit(curDcNo))
            return;
        else {
            return true;
        }
    ...
    wizardObj.createWizard("dcWizard");

    wizardObj.checkClick = function (elem, type) {

        if (isMobile) {
            var dcNo = parseInt(GetWizardDcNo($j("#wizardWrappper #wizardHeader .step.active").data("target"))) + 1;
            var wrapperForEditFields = "wrapperForEditFields" + dcNo;
            try {
        ...
        if (type == "prev" || type == "next") {
            ...
            if (type == "next")
                var validationResult = wizardObj.validateTheKey(validateObj, validateKey);
            else if (type == "prev")
            ...
            if (goingStep > curStep) {
                for (var i = curStep; i < goingStep; i++) {
                    var presValidation = wizardObj.validateTheKey(validateObj, i);
...
function GetWizardDcNo(wizardId) {
    try {
        return wizardId.split("DivFrame")[1];
    } catch (ex) {
        console.log("Error in GetWizardDcNo. Incoming WizardId = " + wizardId);
    }
    return "";
}

function CheckWizardSaveButton() {
    var totalSteps = $("#wizardHeader .step").filter(":visible").length;
    var curStep = $("#wizardHeader .step").filter(":visible").index($("#wizardHeader .step.active"));

    if ($("#wizardHeader .step").filter(":visible").length == 1 || (curStep == (totalSteps - 1)))
        $("#wizardNextbtn").show().attr({
            "onclick": wizardSaveActionBtn,
            "title": "Save"
        }).html(`<span class="material-icons">save</span> <span class="wizard-next">Save</span>`);
...
function ToggleWizardDc(dcNo, action) {
    var elem = $("div#wizardWrappper div#wizardHeader div").find('[data-target="DivFrame' + dcNo + '"]');
    if (elem != undefined) {
        if (action == "hide") {
            var activeDcNo = GetWizardDcNo($j("#wizardWrappper #wizardHeader .step.active").data("target"));
            elem.hide();
            if (dcNo == activeDcNo) {
                if ($j("#wizardPrevbtn").is(":visible"))
                    wizardObj.checkClick(this, 'prev');
                else if ($j("#wizardNextbtn").is(":visible"))
                    wizardObj.checkClick(this, 'next');
            } else
                wizardObj.checkNxtPrevBtns(false);
        } else if (action == "show") {
            elem.show();
            if (!elem.hasClass("complete"))
                elem.nextAll().removeClass("in-progress complete")
        }
    }

    var wizardSteps = $("#wizardHeader .step");
    wizardSteps.removeClass("lastStep");
    wizardSteps.filter(":visible").last().addClass("lastStep");
}
    ...
    if (isCompress === "true" || isCompress === true) {
        $("#wBdr").find(".grid-stack").each(function (ind, gsElem) {
            if ($(gsElem).data('gridstack')) {
                $(gsElem).data('gridstack').cellHeight(45, false);
                $(gsElem).data('gridstack').verticalMargin(1, false);
            }
        });
        if ($("link[href='../Css/compressedTstruct.min.css?v=36']").length != 1)
            $('head').append($('<link rel="stylesheet" type="text/css" />').attr('href', '../Css/compressedTstruct.min.css?v=36'));
        ...
        $("#wBdr").find(".grid-stack").each(function (ind, gsElem) {
            if ($(gsElem).data('gridstack')) {
                $(gsElem).data('gridstack').cellHeight(60, false);
                $(gsElem).data('gridstack').verticalMargin(20, false);
            }
...
function MultiGroupSelectClk(event, el) {
    ...
    if (className == "dropdown-main")// className == "dropdown-option dropdown-chose" || className == "dropdown-option" || className == "dropdown-chose-list" || className == "dropdown-group")
        ...
        if ($("#" + fldNameMSId).parents(".edit-mode-content").length > 0)
            $("#" + fldNameMSId).parents(".edit-mode-content").css({ "position": "absolute", "width": $("#" + fldNameMSId).parents("td").outerWidth(), "left": $("#" + fldNameMSId).parents("td").offset().left });
...
function AxGetFieldData(ParentFieldId) {
    ...
    else if (($("#" + ParentFieldId).prop("type") == "text" && $("#" + ParentFieldId).hasClass("fldAutocomplete")) || ($("#" + ParentFieldId).prop("type") == "textarea" && $("#" + ParentFieldId).attr("data-type").startsWith("autocomplete-") == true)) {
        var custfldData = [];
        var custfldId = [];
        var custfldDep = [];
...
function FieldTypeTable(event, el) {
    let thisFldId = $(el).parents(".autoinput-parent").find("input").attr("id");
...
function ReloadJqueryReference() {
    if ($('script[src^="/Js/formInit"]').length) {
        $('script[src^="/Js/formInit"]').attr("id", "removeInit");
        ...
        $('head script[src*="/autoComplete.min.js"]').remove();
        ...
        var fileref = document.createElement('script');
        fileref.setAttribute("type", "text/javascript");
        fileref.setAttribute("src", "../Js/autoComplete.min.js");
        if (typeof fileref != "undefined")
            document.getElementsByTagName("head")[0].appendChild(fileref);
...
function exportGridToExcel(exportDcNo) {
    var gridExport = {
        processGridExcelTable(containsHeader = true) {
            $("body").append($('#gridHd' + exportDcNo).clone().attr("id", "gridToExport").css({ "position": "fixed", "left": ($(window).width() + 1000) + "px" }));
        ...
        generateExcel(containsHeader = true) {
            ...
            try {
                var workbook = XLSX.utils.table_to_book($('#gridToExport')[0], { sheet: DCName[parseInt(exportDcNo) - 1], display: true, raw: true });
                if (workbook) {
                    ...
                    sheet["!cols"] = objectMaxWidth.map(w => { return { width: w } })
