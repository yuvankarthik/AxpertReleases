...
function GetCancelRemarks(actionName, fileup, manRemarks) {
    ShowPrompt(actionName, fileup, manRemarks, true);
}

function ShowPrompt(actionName, fileup, manRemarks, isRemarksEntered) {
    var msgStatus = true;
    var i = 0;
    try {
    ...
    $j.msgBox({
        ...
        title: "Remarks",
        inputs: [
            isRemarksEntered == false ? { header: cutMsg, type: "label", name: "" } : {},
            AxHeader == undefined ? { header: cutMsgEnter, type: "textarea", name: "remark" } : { header: AxHeader, type: "textarea", name: "remark" },
            { header: cutMsgChar, type: "readonly", value: "150" }],
        buttons: [
            { value: "Ok" }, { value: "Close" }],
        afterClose: function () { if (!msgStatus) ShowPrompt(actionName, fileup, manRemarks, false); },

        success: function (result, values) {

            if (result == "Ok") {
                if (values[0].value == "" && manRemarks == "y") {
                    msgStatus = false;
                }
                else {
                    CallActionExt(actionName, fileup, values[0].value);
                    msgStatus = true;
                }
            }
            else {
...
function CallAction(actionName, fileup, confirmmsg, remarks, manRemarks, dsignaction) {

    try {
        if (window.frameElement.contentWindow.jQuery('#form1').attr('action') != "" && window.frameElement.contentWindow.jQuery('#form1').attr('action').indexOf('&actionflg=change') > -1 && window.frameElement.contentWindow.jQuery('#form1').attr('action').indexOf('&transid=paget') > -1) {
            let _thisUri = window.frameElement.contentWindow.jQuery('#form1').attr('action');
            let _oTransID = decodeURI(_thisUri)
                .split('?')[1]
                .split('&')
                .map(param => param.split('='))
                .reduce((values, [key, value]) => {
                    if (key.toLowerCase() == 'stransid' || key.toLowerCase() == 'fldname' || key.toLowerCase() == 'flddcno' || key.toLowerCase() == 'dbdatatype') {
                        values[key] = value
                    }
                    return values
                }, {});

            let _stransid = _oTransID.stransid;
            let _dcname = _oTransID.flddcno;
            let _fldName = _oTransID.fldname;
            let _dbdatatype = _oTransID.dbdatatype;
            changedFieldDatatype(_stransid, _dcname, _fldName, _dbdatatype);
            if (fldChangeDttype) {
                fldChangeDttype = false;

                if (_oTransID.dbdatatype == "Character" && (actionName == "addwholenumber" || actionName == "adddecimalnumb" || actionName == "adddate" || actionName == "addtime" || actionName == "addmobilenumber" || actionName == "addphonenumber" || actionName == "addcurrency" || actionName == "addpincode" || actionName == "addzipcode" || actionName == "addrandomnumber" || actionName == "adddc" || actionName == "genmap" || actionName == "fillg" || actionName == "mdmaps" || actionName == "actions" || actionName == "addimage" || actionName == "addrichtext")) {
                    showAlertDialog("error", "Data already exists in transaction table, can not change datatype. If you want to change datatype, needs to take backup of the table and delete the data from concerned column.");
                    return;
                } else if (_oTransID.dbdatatype == "Numeric" && (actionName == "adddate" || actionName == "addtime" || actionName == "adddc" || actionName == "genmap" || actionName == "fillg" || actionName == "mdmaps" || actionName == "actions" || actionName == "addimage" || actionName == "addrichtext" || actionName == "addshorttext" || actionName == "addsimpletext" || actionName == "addlargetext" || actionName == "addautogenerate" || actionName == "dropdwnlist" || actionName == "addmultiselect" || actionName == "addchecklist" || actionName == "addcheckbox" || actionName == "addradiogroup" || actionName == "addemail" || actionName == "addurl" || actionName == "addipaddress" || actionName == "addpassword" || actionName =="addtable")) {
                    showAlertDialog("error", "Data already exists in transaction table, can not change datatype. If you want to change datatype, needs to take backup of the table and delete the data from concerned column.");
                    return;
                } else if (_oTransID.dbdatatype == "Date%2fTime" && (actionName == "adddc" || actionName == "genmap" || actionName == "fillg" || actionName == "mdmaps" || actionName == "actions" || actionName == "addimage" || actionName == "addrichtext" || actionName == "addshorttext" || actionName == "addsimpletext" || actionName == "addlargetext" || actionName == "addautogenerate" || actionName == "dropdwnlist" || actionName == "addmultiselect" || actionName == "addchecklist" || actionName == "addcheckbox" || actionName == "addradiogroup" || actionName == "addemail" || actionName == "addurl" || actionName == "addipaddress" || actionName == "addpassword" || actionName == "addtable" || actionName == "addwholenumber" || actionName == "adddecimalnumb" || actionName == "addmobilenumber" || actionName == "addphonenumber" || actionName == "addcurrency" || actionName == "addpincode" || actionName == "addzipcode" || actionName == "addrandomnumber")) {
                    showAlertDialog("error", "Data already exists in transaction table, can not change datatype. If you want to change datatype, needs to take backup of the table and delete the data from concerned column.");
                    return;
                } else if (_oTransID.dbdatatype == "Image" && (actionName == "adddc" || actionName == "genmap" || actionName == "fillg" || actionName == "mdmaps" || actionName == "actions" || actionName == "addrichtext" || actionName == "adddate" || actionName == "addtime" || actionName == "addshorttext" || actionName == "addsimpletext" || actionName == "addlargetext" || actionName == "addautogenerate" || actionName == "dropdwnlist" || actionName == "addmultiselect" || actionName == "addchecklist" || actionName == "addcheckbox" || actionName == "addradiogroup" || actionName == "addemail" || actionName == "addurl" || actionName == "addipaddress" || actionName == "addpassword" || actionName == "addtable" || actionName == "addwholenumber" || actionName == "adddecimalnumb" || actionName == "adddate" || actionName == "addtime" || actionName == "addmobilenumber" || actionName == "addphonenumber" || actionName == "addcurrency" || actionName == "addpincode" || actionName == "addzipcode" || actionName == "addrandomnumber")) {
                    showAlertDialog("error", "Data already exists in transaction table, can not change datatype. If you want to change datatype, needs to take backup of the table and delete the data from concerned column.");
                    return;
                } else if (_oTransID.dbdatatype == "Text" && (actionName == "adddc" || actionName == "genmap" || actionName == "fillg" || actionName == "mdmaps" || actionName == "actions" || actionName == "addimage" || actionName == "adddate" || actionName == "addtime" || actionName == "addshorttext" || actionName == "addsimpletext" || actionName == "addlargetext" || actionName == "addautogenerate" || actionName == "dropdwnlist" || actionName == "addmultiselect" || actionName == "addchecklist" || actionName == "addcheckbox" || actionName == "addradiogroup" || actionName == "addemail" || actionName == "addurl" || actionName == "addipaddress" || actionName == "addpassword" || actionName == "addtable" || actionName == "addwholenumber" || actionName == "adddecimalnumb" || actionName == "adddate" || actionName == "addtime" || actionName == "addmobilenumber" || actionName == "addphonenumber" || actionName == "addcurrency" || actionName == "addpincode" || actionName == "addzipcode" || actionName == "addrandomnumber")) {
                    showAlertDialog("error", "Data already exists in transaction table, can not change datatype. If you want to change datatype, needs to take backup of the table and delete the data from concerned column.");
                    return;
                }
            }
    ...
    if (actionCallFlag == actionCallbackFlag) {
        ...
        $("#icons,#btnSaveTst").css({ "pointer-events": "auto" });
    } else {
    ...
    ShowDimmer(true);
    if (AxActiveAction == "readivdef" || AxActiveAction == "deleteivdef" || AxActiveAction == "saveasivdef" ) {
        ShowDimmer(false);
        var ldrText = AxActiveAction == "deleteivdef" ? "Deleting iview structure and tables.." : "Creating iview structure..";
    ...
    if(proceed && actionName=="iSave" && $j("#recordid000F0").val()!="" && $j("#recordid000F0").val()!="0" && saveValueFlag=="T" && $j("#savevale000F1").length>0 && $j("#savevale000F1").val()=="F")
    {
        let chngFldName=$("#name000F1").val();
        let confirmErmsg="SaveValue of "+chngFldName+" has been changed to false. Field value could not be saved in DcTable.";
        if (!confirm(confirmErmsg)) {
            EnableToolBarBtns();
            actionCallbackFlag = actionCallFlag;
            ShowDimmer(false);
            AxWaitCursor(false);
    ...
    if (proceed) {
        ...
        if (remarks == "y") {
            ...
            GetCancelRemarks(actionName, fileup, manRemarks);
            ShowDimmer(false);
        }
        else {
            CallActionExt(actionName, fileup, '', dsignaction);
        }
    }
}

function CallActionExt(actionName, fileup, remarks, dsignaction) {
    ...
    var dwbIvName = (callParentNew("middle1", "id")?.contentWindow?.dwbIvDefName || callParentNew("dwbIvDefName"));
    var dwbIvColDesignJSON = callParentNew("ivColDesignJSON");
    var dwbIvParamsOrder = callParentNew("ivParamsOrder");
    
    if(callParentNew("isDWB") && dwbIvName && dwbIvColDesignJSON && dwbIvColDesignJSON[dwbIvName]){
        var columnOrder = dwbIvColDesignJSON[dwbIvName].map(col=> [col.Savestr,col.mergeid,col.mergename].join("|")).join();
        // var columnOrder = dwbIvColDesignJSON[dwbIvName].map(col=> col.Savestr).join();
        if(columnOrder){
            txt = txt.replace("<root", `<root changedColumnOrder="${columnOrder}"`);
        }
    }

    if(callParentNew("isDWB") && dwbIvName && dwbIvParamsOrder && dwbIvParamsOrder[dwbIvName]) {
        var ParametersOrder = dwbIvParamsOrder[dwbIvName].map(col=> col.name).join();
        if(ParametersOrder){
            txt = txt.replace("<root", `<root changedParamOrder="${ParametersOrder}"`);
        }        
    }
    
    if(transid == 'ad__d' && recordid!="")
    {
        var dwbdctable = callParentNew("dwbtstdctable");
        if(dwbdctable==$("#tablename000F1").val())
    ...
    var source =  "t";
    if(transid == "sect" || transid == "NF_AG" || transid == "mntss" || transid == "ad_fg" || transid == "ad_s" || transid == "ad_sc"  || transid == "axcad" || transid == "ad_sp" || transid == "ad_ic")
        source="t"+ transid;
    try {

        callBackFunDtls = "CallActionExt♠" + actionName + "♠" + fileup + "♠" + remarks + "♠" + dsignaction;
        ASB.WebService.CallActionWS(ChangedFields, ChangedFieldDbRowNo, ChangedFieldValues, DeletedDCRows, DeletedFieldValue, ArrActionLog, visDcname, txt, fup, source, delRows, chngRows, tstDataId, files, SuccessCallbackAction, OnException);
    }
    catch (exp) {
        actionCallbackFlag = actionCallFlag;
        $("#icons,#btnSaveTst,.wizardNextPrevWrapper").css({ "pointer-events": "auto" });
...
function ExecRefreshIviewRows(rowData) {
    var refreshRowData = {
        ...
        getActionEvent() {
            _this.event = ((eventObj = ivActions[$j("#hdnAct").val().substr(4)]) && eventObj["@apply"]) || "Current Row";
            return _this.event;
        },
        process(rowData) {
            _this = this;
...
function ExecData(dataJsonObj, calledFrom) {
    ...
    for (var i = 0; i < dataJsonObj.length; i++) {
        ...
        if (fldType == "dc") {
            ...
            var fName = GetFieldsName(fldName);
            if ((typeof AutosaveDraft != "undefined" && AutosaveDraft == "true" && checkIsdraft == "false") || (typeof AutosaveDraft != "undefined" && AutosaveDraft == "" && checkIsdraft == "false")) {
            ...
            if (fld.length > 0) {
                ...
                if (fldType == "m") {
                    ...
                    if (transid == "mntss" && fldName == 'sql_editor_test000F2' && recordid != "0") {
                        sql_editor_test = fldValue;
                    }
                }
                    ...
                    if (calledFrom == "GetDep" && (i + 1) == dataJsonObj.length) {
                        ClearChecklistOnGetDep(comboFld);
                        comboFld = "";
                        comboVal = "";
                    }
                    ...
                    if (fldValue != "") {
                        ...
                        $j("#" + fldName).attr("src", "../AxpImages/upload.png");
                    }
                }
                else if (fldType == "b") {
                    ...
                    if (fld.attr("type") == "hidden") {
                        ...
                        if ($j("#" + fldName).hasClass("fldAutocomplete"))
                            fldIsAutoComp = true;
                        ...
                        if (comboField != undefined) {
                            ...
                            if (comboField.prop("type") == "select-one") {
                                ...
                                if (IsChkList) {
                                    ...
                                    fldClass = "type=radio class='multiFldRdg'";
                                }

                                var tmpFld; var tmpID = "";
                                //$j(spanClass).each(function () {
                                //    if ($j(this).attr("name") == comboFld) {
                                //        tmpID = $j(this).attr("name");
                                //        tmpFld = $j(this);
                                //        return false;
                                //    }
                                //});
                                ...
                                //if (IsChkList) {
                                //    chkHtml = "<span><input " + fldClass + " id='" + chkId + "' onchange='CallMainBlur(this)'  name='" + chkId + "' title='" + fldHint + "' value='" + fldclValue + "' " + strChecked + " ></input>" + fldclText + align + "</span>";
                                //}
                                //else {
                                //    chkHtml = "<span><input " + fldClass + " id='" + fldName + "' onchange='CallMainBlur(this)' name='" + fldName + "' title='" + fldHint + "' value='" + fldclValue + "' " + strChecked + " ></input>" + fldclText + align + "</span>";
                                //}
                                ...
                                if (IsDcGrid(dcNo) || comboField.attr("type") == "radio") {
                                    ...
                                    //} else {
                                    //    tokens = $("#" + tmpID).data("valuelist").split($("#" + tmpID).data("separator"));
                                    //}
                ...
                if (fldType != "dc" && fldType != "dv") {
                    ...
                    if (calledFrom == "PopGridCombos" || calledFrom == "Action" || calledFrom == "GetDep")
    ...
    for (var i = 0; i < attJsonObj.length; i++) {
        ...
        for (var mm = 0; mm < responsemsgFlds; mm++) {
            if (fileonloadarray.indexOf(attach[mm].toString()) == -1) {
                ...
                var attachstr = CheckUrlSpecialChars(attach[mm].toString());
                attachstr = '"' + attachstr + '"';
                var attcount = parseInt(mm, 10) + 1;
                if (mm == 0)
                    attachments += "<div class=\"row attachmentData\">";

                attachments += "<div class=\"col-md-4 col-sm-6\"><div class=\"attachments\"><a class=linkrem alt='Remove File' onclick='javascript:RemoveFile(" + attachstr + "," + rid + ");'><img class='close pull-right' src='../axpimages/icons/16x16/cross.png' alt='delete' /></a>&nbsp;<a id=loadattach" + attcount + " class=\"pull-left Attachlink flagcount\" style=\"cursor:pointer;\" onclick='javascript:OpenAttachment(" + attachstr + "," + rid + ");'>" + fileonloadarray[mm].toString() + "</a></div></div>";

                if (mm == responsemsgFlds - 1) {
                    attachments += "</div>"
                }
            }
        }
        if (attachments != "") {
            ...
            hdnattachcount += "<input id=hdncount type=hidden value=" + mm + " >"
            $j("#attachment-overlay").show();
            $j("#attachment-overlay").css("display", "block !important");
            $j("#attachment-overlay").removeClass("hide");

            $j("#attachment-overlay").html(attachments + hdnattachcount);
        }
    }
}
            ...
            if (btn != "") {
                ...
                for (var k = 0; k < btnarr.length; k++) {
                    if (btnarr[k] == "a") {
                        ...
                        $j("#selectbox").append("<li onclick=\"SlectBoxWrf('Approve')\" class=\"Selectboxlist\" id=\"Approve\">Approve</li>");
                    }
                    else if (btnarr[k] == "r") {
                        $j("#btntabreject").prop("disabled", false);
                        $j("#btntabreject").removeClass("disablewfbuttons").addClass("enablewfbuttons");
                        $j("#selectbox").append("<li onclick=\"SlectBoxWrf('Reject')\" class=\"Selectboxlist\" id=\"Reject\">Reject</li>");
                    }
                    else if (btnarr[k] == "t") {
                        $j("#btntabreturn").prop("disabled", false);
                        $j("#btntabreturn").removeClass("disablewfbuttons").addClass("enablewfbuttons");
                        $j("#selectbox").append("<li onclick=\"SlectBoxWrf('Return')\" class=\"Selectboxlist\" id=\"Return\">Return</li>");
                    }
                    else if (btnarr[k] == "v") {
                        $j("#btntabreview").prop("disabled", false);
                        $j("#btntabreview").removeClass("disablewfbuttons").addClass("enablewfbuttons");
                        $j("#selectbox").append("<li onclick=\"SlectBoxWrf('Review')\" class=\"Selectboxlist\" id=\"Review\">Review & Forward</li>");
                    }
                    isBtnDis = true;
                }
            }
    ...
    for (var i = 0; i < cmdJsonObj.length; i++) {
        ...
        if (cmdName == "openfile" || cmdName == "printfile") {
            ...
            if (SetCarryFlds.length > 0) {
                ...
                querySCFld += "&act=open";
                AvoidPostBackAfterSave("tstruct.aspx?transid=" + transid + querySCFld);
            }
            else
                AvoidPostBackAfterSave("tstruct.aspx?transid=" + transid);
            ...
            try {
                if (curFrame.attr("id") === "axpiframe") {
                    ...
                    var url = "./tstruct.aspx?&transid=" + tst;
            ...
            if(tstid=="mntss" && tparams.indexOf("&actionfld=change")>-1) {
                callParentNew("actionfldflag=","change");
            }
            else
                callParentNew("actionfldflag=","");
            ...
            if ((axpConfigNavType == undefined || axpConfigNavType == "") && parFrm.hasClass("frameSplited") && (tstid != "ad_tb" && tstid != "ad_tg"))
                popup = "split";
            ...
            if (popup == "pop") {
                ...
                var openpopup = 'tstruct.aspx?act=open&transid=' + tstid + tparams + designParam;
                var popupWidth = (tstid == "ad_tg" ? "80vw" : "") || "";
                if (tstid == "ad_as")
                    callParentNew("createPopupdesign")(openpopup);
                else
                    createPopup(openpopup, "", popupWidth);
            }
            else if (popup == "newpage") {
                popupFullPage("tstruct.aspx?act=open&transid=" + tstid + tparams + designParam);
            }
            else if (popup != "split") {
                var redirectionLink = "tstruct.aspx?act=open&transid=" + tstid + tparams + designParam;

                if (tstid == "ad_sp") {
                    redirectionLink = "tstructnew.aspx?act=open&transid=" + tstid + tparams + designParam;
                }
                
                var customCalled = false;
                try {
                    customCalled = loadInMiddle1(redirectionLink, cmdJsonObj);
                } catch (ex) { }
                if (!customCalled)
                    window.document.location.href = redirectionLink;
            }
            else if (popup == "split") {
                    callParentNew(`OpenOnPropertyBase(${"tstruct.aspx?act=open&transid=" + tstid + tparams + designParam})`, 'function');
                }
            ...
            if (popup == "pop") {
                ...
                var loadpopup = 'tstruct.aspx?act=load&transid=' + tstid + tparams;

                if (tstid != "ad_tb" && tstid != "ad_tg" && tstid !="ad_as") {
                    createPopup(loadpopup);
                }
                else {
                    callParentNew("createPopupdesign")(loadpopup);
                }
                
            }
            else if (popup == "newpage") {
                popupFullPage("tstruct.aspx?act=load&transid=" + tstid + tparams);
            }
            else if (popup != "split") {
                window.document.location.href = "tstruct.aspx?act=load&transid=" + tstid + tparams;
            }
            else if (popup == "split") {
                if(appGlobalVarsObject._CONSTANTS.callerWindow){
                    appGlobalVarsObject._CONSTANTS.callerWindow.renderFormProperties("tstructnew.aspx?act=load&transid=" + tstid + tparams);
                }else{
                    callParentNew(`OpenOnPropertyBase(${"tstruct.aspx?act=load&transid=" + tstid + tparams})`, 'function');
                }
            }
        }
            ...
            if ((axpConfigNavType == undefined || axpConfigNavType == "") && parFrm.hasClass("frameSplited") && (ivn != "toolbgrp"))
                popup = "split";
            else if (axpConfigNavType && axpConfigNavType != "") {
            ...
            if (popup == "pop") {
                ...
                var popupWidth = (ivn == "toolbgrp" ? "80vw" : "") || "" ;
                createPopup(loadpopup, "", popupWidth);
                ...
                if (linkParams != "") {
                    ...
                    callParentNew("updateAppLinkObj(" + srcUrl + ",1)", "function");
                }
            ...
            if (pgname != "" && pgname.toLowerCase().startsWith("pagets")) {
                ...
                window.document.location.href = "tstruct.aspx?act=open&transid=" + pgname + tparams;
            }
            ...
            if (pgname != "" && pgname.toLowerCase().startsWith("pagets")) {
                pgname = pgname.substring(6);
                window.document.location.href = "tstruct.aspx?act=load&transid=" + pgname + tparams;
            }
            else if (pgname != "" && pgname.toLowerCase().startsWith("pageiv")) {
                pgname = pgname.substring(6);
                window.document.location.href = "ivtoivload.aspx?ivname=" + pgname + "&" + tparams;
...
function redirectOnSaveAction() {
    ...
    if (recIdVal == "" || recIdVal == "0") {
        window.location.href = "tstruct.aspx?act=load&transid=" + tst + axSplit;
    }
    else {
        saveActRecSuccess = true;
        if (AxIsTstructCached) {
            ...
            if(recIdVal && transid == "axcad"){
                clearCardCache(recIdVal);
            }

            var qshltype="";
            try{
                let axpifrm = callParentNew("axpiframe", "id");
                if(typeof axpifrm!="undefined"){
                    let qshltp= getqsParameterByName("hltype",axpifrm.src);
                    qshltype=qshltp==null?"":qshltp;
                }
            }catch(ex){}

            if (callParentNew("isDWB") && ((tst == "mntss" || tst == "NF_AG" || tst == "ad_s" || tst == "ad__d" || tst == "ad_ge" || tst == "ad_md" || tst =="ad_fg") && qshltype!="load")) {//|| tst == "ad__t"
                var dwbPrms = callParentNew("dwbtstparams");
                if (tst == "mntss" || tst == "NF_AG")
                { 
                    callParentNew("dwbtstparams=", dwbPrms + "&oldrecid=" + recIdVal); 
                } 
                //else 
                //{ 
                //    callParentNew("dwbtstparams=", dwbPrms + "&oldferecid=" + recIdVal); 
                //}
                callParentNew(`OpenOnPropertyBase(${"tstruct.aspx?act=open&transid=paget" + callParentNew("dwbtstparams")})`, 'function');
                // else
                //     callParentNew(`OpenOnPropertyBase(${"tstruct.aspx?act=open&transid=paget" + callParentNew("dwbtstparams")})`, 'function');
            }
            else
                window.location.href = "tstruct.aspx?act=load&transid=" + tst + "&recordid=" + recIdVal + axSplit;
        }
    }
}
...
function CopyFromMasterRow(fldId, dcNo, masterRow, calledFrom, keyColValue, isPopGrid) {
    ...
    if (IsDcGrid(dcNo)) {
        ...
        if (cFld.attr("type") == "select-one" || cFld.prop("type") == "select-one") {
            ...
            if (!$j("#" + fldId).hasClass("fldAutocomplete") && !$j("#" + fldId).hasClass("fldmultiSelect"))
    ...
    if (fldName != null && fldName != undefined) {
        ...
        if (fldName.indexOf("axpvalid") != -1) {
            ...
            if (value != "" && value != undefined) {
                ...
                if (strValue == "B")
                    DisableDeleteBtn("del" + rowNo + "F" + dcNo, strValue);
...
function DisableDeleteBtn(delBtnId, strValue) {
    delImg = $j("#" + delBtnId);
    if (delImg.length > 0) {
        $j(this).prop("disabled", true);
        delImg.css('cursor', 'default');
        if (strValue == "C" || strValue == "B") {
            delImg.addClass("disabledelete");
            delImg.unbind('click');
        }
        else {
...
function ConstructAttachments() {
    ...
    $j("#attachment-overlay").css("display", "block !important;");
    $j("#attachment-overlay").removeClass("hide");
    $j("#attachment-overlay").html(attachments + hdnattachcounts);
...
function ClearImageSrc(delObj) {
    ...
    $j("#" + imgId).attr("src", "../AxpImages/upload.png");
    ...
    for (var i = 0; i < ErroMsgJsonObj.length; i++) {
        ...
        if (errMsg != null && errMsg != undefined && errMsg != "") {
            ...
            if (errFld) {
                ...
                var rowNo = GetClientRowNo(fldDetails[1], dcNo);
...
function getComntWf() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#btnWrfSave").addClass("hide");
            $("#btnWrfCancel").addClass("hide");
        },
        error: function (data) {
            console.log(data);
        }
...
function getWfLevelUSsrs(lvlIdx, maxLevels, usernames, statusWrk, elno, levelJson) {
    ...
    //if (parent.nodeApi != '') {
    //    $.ajax({
    //        url: parent.nodeApi + 'getWorkflow',
    //        type: 'POST',
    //        data: { transid: tst, level: (lvlIdx + 1), rcId: recid, utl: parent.utl, session_id: parent.mainSessionId, username: parent.mainUserName, authorization: parent.nodeAccessToken, appSKey: appsessionKey },
    //        dataType: 'json',
    //        success: function (data) {
    //            if (data.status == true) {
    //                var dataUn = data.data;
    //                AllUsernames = $.map(dataUn, function (el) { return el });
    //                var tempIdx = lvlIdx;
    //                for (var i = 0; i < AllUsernames.length; i++) {
    //                    usernames[tempIdx] = AllUsernames[i];
    //                    tempIdx += 1;
    //                }
    //            }
    //            var htmlwrk = constructWrkHtml(maxLevels, usernames, statusWrk, elno, levelJson);
    //            $(htmlwrk).insertAfter($("#stratWrkf"));
    //            popoverWrkFl();
    //            ShowDimmer(false);
    //            WrkflFlag = false;
    //        },
...
function constructWrkHtml(maxLevels, usernames, statusWrk, elno, levelJson) {
    ...
    for (var l = 0; l < parseInt(maxLevels) ; l++) {
        ...
        if (typeof levelJson[levelno] != "undefined" && levelJson[levelno].lno != (l + 1) && $j("#hdnWfLno").val() != "-1")
            continue;
        htmlwf += '<div class="workflowMsg workflowdisplayinline">';
        ...
        $('#stratWrkf')[0].innerHTML = 'Workflow';
        ...
        if (status.indexOf('rejected') == -1 & status.indexOf('returned') == -1)
            htmlwf += '<div class="msglable workflowdisplayinline wrkflw' + status + '" data-toggle="popoverone" data-trigger="hover" style="text-transform:capitalize;" data-placement="bottom" data-content="';
        else {
            htmlwf += '<div class="msglable workflowdisplayinline wrkflw' + status + '" data-toggle="popoverone" data-trigger="hover" style="text-transform:capitalize;" data-placement="bottom" data-content="';
        }
        ...
        if (statusWrk[l] == '') {
            status = "Pending";
            htmlwf += capitalizeFirstLetter(status) + (usernames[l] != '' ? (status.toLowerCase() == 'pending' ? ' with ' : ' by ') + usernames[l] : '') + '">';
        }
        else if (typeof levelJson[levelno] != "undefined" && levelJson[levelno].lno == elno && parseInt(elno) == (l + 1))
            htmlwf += $j("#lblStatus").text() + '">';
        else

            htmlwf += status + (status == 'Pending' ? ' with ' : ' by ') + usernames[l] + '">';

        if (status === "notapplicable")
            status = "NA";
        if (status == "approved") {
            htmlwf += '<span class="para"><i class="icon-arrows-circle-check wrkflwicons"></i> L' + (l + 1) + '</span>';
        } else if (status == "Pending") {
            htmlwf += '<span class="para"><i class="icon-basic-clock wrkflwicons"></i> L' + (l + 1) + '</span>';
        } else if (status == "rejected") {
            htmlwf += '<span class="para"><i class="icon-arrows-circle-remove wrkflwicons"></i> L' + (l + 1) + '</span>';
        } else if (status == "reviewed") {
            htmlwf += '<span class="para"><i class="icon-arrows-circle-left wrkflwicons"></i> L' + (l + 1) + '</span>';
        } else if (status == "returned") {
            htmlwf += '<span class="para"><i class="icon-arrows-circle-right wrkflwicons"></i> L' + (l + 1) + '</span>';
        }

        htmlwf += '</div></div>';
        for (var j = 0; j < levelJson.length; j++) {
            if (levelJson[j].lno == elno && parseInt(elno) == (l + 1) && status == "Pending") {
                htmlwf += '<div class="downArr workflowdisplayinline" id="workflowdropdown"><i class="workflowOptions icon-arrows-down downarrow" title="select"></i></div>';
                break;
            }
        }
        if (usernames[l].indexOf(',') > -1)
            htmlwf += '<div class="itemone workflowdisplayinline"  data-content="' + usernames[l] + '" data-toggle="popoverone" data-trigger="hover" data-placement="bottom">' + usernames[l].split(',').length + '</div>';
        levelno++;
    }
    return htmlwf;
}
...
function changedFieldDatatype(_stransid, _dcname, _fldName, _dbdatatype) {
    $.ajax({
        type: "POST",
        url: "../WebService.asmx/CallExecuteSQLFldSN",
        cache: false,
        async: false,
        contentType: "application/json",
        data: JSON.stringify({ transId: _stransid, dcName: _dcname, fldName: _fldName, fldDtType: _dbdatatype }),
        dataType: "json",
        success: function (data) {
            try {
