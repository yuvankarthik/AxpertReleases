...
function createFormSelect(fld) {
    ...
    formSelect.select2({
        ...
        if (typeof $(this).data("separator") != "undefined" && fldAcValue.length > 0) {
        ...
        if (select2IsFocused) {
            ...
            if ($(this).attr("id") != AxFocusedFld && AxFocusedFld != "" && !$("#" + AxFocusedFld).hasClass("fldFromSelect"))// Changed text field and directlly click on select2 field using mouse changed field not getting call mainblur.
            {
        ...
        if ($(this).hasClass('multiFldChk')) {
            ...
            if (curDropdown.find(".select2-results").find(".msSelectAllOption").length == 0) {
                let selectAllHTML = `<div class="msSelectAllOption form-check form-check-custom align-self-end px-5">
...
function createFormMultiSelect(msfldId) {
    ...
    formSelect.select2({
        ...
        if (typeof $(this).data("sep") != "undefined" && fldMulValue.length > 0) {
