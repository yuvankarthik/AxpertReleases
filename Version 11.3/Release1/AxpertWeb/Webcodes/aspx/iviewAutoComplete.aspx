...
<head runat="server">
    ...
    <script type="text/javascript" src="../Js/tstruct.min.js?v=576"></script>
    ...
    <script type="text/javascript">
        ...
        function loadParent(a) {
            try {
                ...
                try {
                    $(callParentNew(`${fldname}`, "id")).empty().append($(gov).find("option").clone());
                    $(callParentNew(`${fldname}`, "id")).val(thisVal);
                    $(callParentNew(`${fldname}`, "id")).trigger("change");
                    callParentNew("loadPopUpPage", "id").dispatchEvent(new CustomEvent("close"));
                    parent.$(callParentNew(`${fldname}`, "id")).data("select2").dropdown.$search.val(txtSrchText.value || txtfilter.value || "");
                    parent.$(callParentNew(`${fldname}`, "id")).data("select2").dropdown.$search.val("");
                } catch (ex) {}
            } catch (ex) {}
        }
    </script>
    ...
    <script src="../Js/iviewAutoComplete.min.js?v=2" type="text/javascript"></script>
</head>
