...
function callserverbtn(btntype) {
    if (btntype == "btnsearch") {
        ...
        if (filterValue == "") {
            callParentNew(`showAlertDialog("warning", "Filter value cannot be left empty.")`, "function");
            return false;
        } else if (filterValue.length <= 2) {
            callParentNew(`showAlertDialog("warning", "Enter at least 3 minimum characters to search.")`, "function");
            return false;
        }
    }
...
function loadParent(a) {
    try {
        ...
        parent.$(callParentNew(`${fldname}`, "id")).val(thisVal).trigger("change");
