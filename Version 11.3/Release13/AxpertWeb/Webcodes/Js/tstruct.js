...
function BindAxpFilesonDep(_thisFldName, _thisfldValue) {
    ...
    if (fuInpVal != "") {
        ...
        $.each(fuInpVal.split(','), function (i, val) {
            ...
            $(file.previewElement).find(".dropzone-filename").attr("onclick", "ShowAxpFileuploadLink('" + $(_myDropzone.files[0].previewElement).parents(".dropzone").attr("id").substr(9) + "','" + _fullval + "','" + filePath + "')");
            if (isthisReadOnly)
