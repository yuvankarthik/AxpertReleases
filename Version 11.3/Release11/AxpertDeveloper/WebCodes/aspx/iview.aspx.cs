...
public partial class iview : System.Web.UI.Page
{
        ...
        try
        {
            ...
            else
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    if (!(iName == "mainrepo" && Session["AxConfigPage"] != null && Session["AxConfigPage"].ToString().StartsWith("addform~")))
                    {
                ...
                ScriptManager.RegisterStartupScript(this, this.GetType(), "ivirHeaderData", "var ivirHeaderData = " + jsonForGrid + ";var getAjaxIviewData=" + getAjaxIviewData.ToString().ToLower() + ";var isListView=" + (objIview.purposeString == "" ? "false" : "true") + ";var showParam=" + objParams.showParam.ToString().ToLower() + ";var iviewWrap = " + util.IviewWrap.ToString().ToLower() + ";var isIviewPopup = " + IsIviewPop.ToString().ToLower() + ";var toolbarJSON=JSON.stringify(" + toolbarJsonString + ");var requestJSON = " + objIview.requestJSON.ToString().ToLower() + ";var redisLoadType = '" + redisLoadType.ToString() + "';var redisLoadKey = '" + redisLoadKey.ToString() + "';var breadCrumbStr = '" + objIview.Menubreadcrumb.ToString() + "';var iviewButtonStyle = '" + iviewButtonStyle.ToString() + "';var inputControlType = '" + inputControlType.ToString() + "';", true);
    ...
    private void GetAxpStructConfig()
    {
        ...
        catch (Exception ex)
        {
            ...
            logObj.CreateLog("GetAxpStructConfig -" + ex.Message, sessID, "GetAxpStructConfig", "new");
        }
    ...
    private void GetIviewStructure(string ivname)
    {
        ...
        try
        {
            ...
            if (isCache)
            {
                logobj.CreateDirectDBLog("RedisCachedStructure- " + ivname, "GetIviewStructure", "", "IView Name :  " + ivname, "Success --" + ivname
            ...
            {
                logobj.CreateDirectDBLog("GetIviewStructure- " + ivname, "GetIviewStructure", "", "IView Name :  " + ivname, "Success --" + ivname + "HasParam :"
        ...
        catch (Exception ex)
        {
            ...
            logobj.CreateDirectDBLog("GetIviewStructure- " + ivname, "GetIviewStructure", errMsg, "IView Name :  " + ivname, "Fail - " + ivname + " - StructureXML");
    ...
    private bool GetIviewDataDBCall(string inputXml, string pageNo, string pageSize)
    {
        ...
        try
        {
            ...
            logobj.CreateDirectDBLog("GetIViewData DirectDB Call- " + iName, "GetIviewDataDBCall", "", iName + "- Sql query "
        ...
        catch (Exception ex)
        {
            ...
            logobj.CreateDirectDBLog("GetIViewData DirectDB Call- " + iName, "GetIviewDataDBCall", errMsg, iName + "- Sql query "
    ...
    private void SetParamValues()
    {
        if (Request.Form.Count > 0 && Request.Form["redisLoadKey"] == null)
        {
            ...
            {
                if (Request.Form.Keys[i].ToString() != "pop" && Request.Form.Keys[i].ToString() != "AxHypTstRefresh")
                {
    ...
    public void GetParams()
    {
        ...
        if (!objIview.isObjFromCache)
        {
            ...
            string strPorj = proj;
            if (iName == "axpubls" || iName == "publist" || iName == "pservers" || iName == "publsdtl")
                strPorj = HttpContext.Current.Session["webProj"].ToString();
            iXml = "<root " + objIview.purposeString + " name =\"" + iName + "\" axpapp = \"" + strPorj + "\" sessionid = \"" + sid + "\" appsessionkey=\"" + HttpContext.Current.Session["AppSessionKey"].ToString() + "\" username=\"" + HttpContext.Current.Session["username"].ToString() + "\" trace = \"" + errLog + "\" firsttime=\"" + (!objIview.requestJSON ? callParamPlusStructure.ToString().ToLower() : "true") + "\"  >";
            //GetParams input xml will contain the query string values passed by previous iview
            ConstructParamXml();
            iXml += "<params>" + pXml + "</params>";
            if (iName == "axpubls" || iName == "publist" || iName == "pservers" || iName == "publsdtl")
                iXml += HttpContext.Current.Session["webAxApps"].ToString();
            else
                iXml += Session["axApps"].ToString();
            iXml += Application["axProps"].ToString() + HttpContext.Current.Session["axGlobalVars"].ToString() + HttpContext.Current.Session["axUserVars"].ToString() + "</root>";
            ...
            if (!objIview.requestJSON)
            {
                    ...
                    if (ires != string.Empty && callParamPlusStructure)
                    {
                        ...
                        if (splitRes.Length == 2)
                        {
                            ...
                            if (istructure != string.Empty)
                            {
                                ...
                                objIview.StructureXml = istructure;
                                isCache = false;
                                logobj.CreateDirectDBLog("CallGetParams Plus Structure(firsttime=true)- " + iName, "GetParams", "", "IView Name :  " + iName, "Success --" + iName
                ...
                if (!cacheParamsXML)
                {
                    logobj.CreateDirectDBLog("RedisCachedParams- " + iName, "GetParams", "", "IView Name :  " + iName, "Success --" + iName
                ...
                if (isCache)
                {
                    logobj.CreateDirectDBLog("RedisCachedStructure- " + iName, "GetParams", "", "IView Name :  " + iName, "Success --" + iName
                                                             + Environment.NewLine + "ISCached : " + isCache.ToString() + " - StructureXML : "
                                                             + Environment.NewLine + Environment.NewLine + objIview.StructureXml);
                }
            ...
            else
            {
                ...
                if (ires != string.Empty)
                {
                    ...
                    logobj.CreateDirectDBLog("Call Params Plus Access Structure Config" + iName, "GetParams", "",
        ...
        else
        {
            ...
            if (hdnGo.Value != "clear")
            {
                ...
                else
                {
                            ...
                            if (cols != "" && hyp != "")
                            {
                                ...
                                if (resultSplitter.Length > 1)
                                {
                                    try
                                    {
                                        ...
                                        logobj.CreateDirectDBLog("Call ListView Cols Plus Structure" + iName, "GetData", "",
                                            "IView Name :  " + iName, "Success --" + iName + Environment.NewLine + Environment.NewLine + Environment.NewLine +
                    ...
                    {
                        logobj.CreateLog("Iview Data Parsing: " + ex.Message, sid, "Iview Data Parse-" + iName, "new");
                    }
                    //}
    ...
    private void parseError(string data)
    {
        ...
        if (errMsg != string.Empty)
        {
            ...
            else
            {
                ...
                Response.Redirect(util.ERRPATH + errMsg, false);
            }
        }
    }
    ...
    public void CallWebservice(string pageNo, string firstTime)
    {
            ...
            if (objIview.IsDirectDBcall)
            {
                ...
                else
                {
                    ...
                    if (objIview.getIviewRowCount || cacheIviewInDT || Convert.ToInt32(gridPageSize) == 0)
                    {
                        ...
                        if (ires == "")
                        {
                            logobj.CreateLog("CallGetIViewWS-" + ires + "-InputXMl-" + iXml, Session["nsessionid"].ToString(), "CallGetIViewWS-Empty-" + String.Format("{0:dMyyyy HHmmss}", DateTime.Now), "new", "true");
                        }
                    }
                    else
                    {
        ...
        else
        {
            ...
            {
                logobj.CreateLog("Custom Iview data call before BindDataGrid " + ex.Message, sid, "AxBeforeIViewBindDataGrid", string.Empty);
            }
    ...
    private XmlDocument GetResultToXmlDoc(string ires, string pageNo)
    {
        ...
        try
        {
            ...
            {
                fileName = "openiview-fromcache-" + iName;
    ...
    private void BindDataGrid(string a, int totRows, string pageNo)
    {
        ...
        if (IsSqlPagination == "true")
        {
            ...
            if ((objIview.getIviewRowCount || objIview.IsDirectDBcall) || (!objIview.getIviewRowCount && !objIview.IsDirectDBcall && objIview.lastPageCached))
                ...
                lblNoOfRecs.Text = " of <a  href=\"javascript:void(0)\" onclick=\"getIviewRecordCount();\" id=\"getIviewRecordCountVal\" title=\"\" class=\"icon-arrows-question fa\"></a>";
    ...
    protected void GridView1_RowDataBound(object sender, System.Web.UI.WebControls.GridViewRowEventArgs e)
    {
        ...
        else if (e.Row.RowType == DataControlRowType.DataRow)
        {
            if (e.Row.DataItem != null)
            {
                ...
                {
                    Response.Redirect(util.ERRPATH + "Error occured while Setting IView hyperlinks");
                }
                ...
                {
                    Response.Redirect(util.ERRPATH + "Error occured in formatting the IView column");
                }
            }
        }
                                ...
                                foreach (XmlNode taskItem in taskListNodes)
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(Lsttask) & string.IsNullOrEmpty(action))
                                    {
                                        ...
                                        else if (Lsttask == "save as")
                                        {
                                            ...
                                            taskJO.Add("href", "javascript:void(0);");
                                            taskJO.Add("onclick", JObject.Parse("{\"PDF\": \"javascript:toPDF('" + iName + "'," + (objIview.purposeString == "" ? "'Iview'" : "'lview'") + ");\", \"Print\": \"javascript:toHTML('" + iName + "'," + (objIview.purposeString == "" ? "'Iview'" : "'lview'") + ",true);\"}"));
                                        }
    ...
    private void ConstructActionBtns()
    {
        ...
        if (actionBtns.Count > 0)
        {
            if (groupButtons != "")
            {
                ...
                {
                    logobj.CreateLog("Iview - Grouped Button -\r\nValue: " + groupButtons + "\r\nException: " + ex.Message + "", HttpContext.Current.Session["nsessionid"].ToString(), "Iview - Grouped Button", string.Empty);
                }
            }
            else
            {
    ...
    private void AlignToolbarBtns()
    {
        ...
        //    //"<li id='filterColumnSelect' class='hide'></li>" +
    ...
    private void ActionClick(string btnId)
    {
    ...
    private void ConstructBreadCrumb()
    {
        ...
        if (language.ToLower() == "arabic")
            direction1 = "right";
        if (language.ToLower() == "arabic")
            strBreadCrumbBtns = "<div id='backforwrdbuttons' class='hide backbutton " + direction1 + " '><span class='navLeft icon-arrows-left-double-32 handCur' onclick='javascript:BackForwardButtonClicked(\"back\");' id='" + "goback" + "' style=\"border:0px;color:black;\" title=\"Click here to go back\" ></span></div>";
        else
            strBreadCrumbBtns = "<div id='backforwrdbuttons' class='hide backbutton " + direction1 + " '><span class='navLeft icon-arrows-left-double-32 handCur' onclick='javascript:BackForwardButtonClicked(\"back\");' id='" + "goback" + "' style=\"border:0px;color:black;\" title=\"Click here to go back\" ></span></div>";
    ...
    private void ConstructParamsHtml(string result)
    {
        ...
        if (!objIview.isObjFromCache)
        {
            ...
            {
                baseDataNodes = productNode.ChildNodes;
                paramHtml.Append("<table width=100% id=\"ivParamTable\"><tr>");
                ...
                foreach (XmlNode baseDataNode in baseDataNodes)
                {
                    ...
                    strJsArrays.Append("parentArr[" + fldNo + "]='" + parameterName + "';typeArr[" + fldNo + "]='" + paramMOE + "';paramType[" + fldNo + "]='" + paramType + "';depArr[" + fldNo + "]='" + paramDepStr + "';hiddenArr[" + fldNo + "]='" + paramHidden + "';Expressions[" + fldNo + "]='" + expr.Replace("'", "\\'") + "';exprSuggestions[" + fldNo + "]=" + exprSuggestion.ToString().ToLower() + ";pCurArr[" + fldNo + "]= '" + paramValue + "';vExpressions[" + fldNo + "]='" + vExpr + "'; ");
                    ...
                    if (paramHidden == "false")
                    {
                        ...
                        if ((paramMOE.ToLower() == "accept") & (paramType == "Date/Time") & (!string.IsNullOrEmpty(expr)))
                        {
                            ...
                            {
                                paramHtml.Append("<td><p class='paramtd1 cap '>" + paramCaption + "</p><p class=paramtd2><input type=text id='" + parameterName + "' name='" + parameterName + "' data-caption='" + paramCaption + "' value='" + paramValue + "' class='tem Family date form-control' onfocus=\"ExprHandler(this.name,this.value, true);" + (isMobile ? "blur();" : "") + "\"></p></td>");
                                //'width: 72%;width: -webkit-calc(80% - 24px);width: -moz-calc(80% - 24px);width: calc(80% - 24px);
                            }
                            else
                            {
                                paramHtml.Append("<td><p class='paramtd1 cap '>" + paramCaption + "</p><p class=paramtd2><input type=text id='" + parameterName + "' name='" + parameterName + "' data-caption='" + paramCaption + "' value='" + paramValue + "' class='tem Family date form-control'" + CallValidateExpr + "" + hideMobKeyBoard + "></p></td>");
                        ...
                        else if ((paramMOE.ToLower() == "accept") & (!string.IsNullOrEmpty(expr)))
                        {
                            ...
                            {
                                paramHtml.Append("<td><p class='paramtd1 cap '>" + paramCaption + "</p><p class=paramtd2><input type=text id='" + parameterName + "' name='" + parameterName + "' data-caption='" + paramCaption + "' value='" + paramValue + "' class='" + onlyTime + "tem Family form-control ' onblur='valdDecPts(this," + decPts + ")' onfocus=\"ExprHandler(this.name,this.value, true);" + (isMobile ? "blur();" : "") + "\" " + CallValidateExpr + "></p></td>");
                            }
                            else
                            {
                                paramHtml.Append("<td><p class='paramtd1 cap '>" + paramCaption + "</p><p class=paramtd2><input type=text id='" + parameterName + "' name='" + parameterName + "' data-caption='" + paramCaption + "' value='" + paramValue + "' class='" + onlyTime + "tem Family form-control ' onfocus=\"ExprHandler(this.name,this.value, true);" + (isMobile ? "blur();" : "") + "\" " + CallValidateExpr + "></p></td>");
                            }
                        ...
                        else if ((paramMOE.ToLower() == "accept"))
                        {
                            if (paramType.ToLower() == "numeric")
                            {
                                paramHtml.Append("<td><p class='paramtd1 cap '>" + paramCaption + "</p><p class=paramtd2><input type=text id='" + parameterName + "' name='" + parameterName + "' data-caption='" + paramCaption + "' value='" + paramValue + "' class='" + onlyTime + "tem Family form-control ' onblur='valdDecPts(this," + decPts + ")' " + CallValidateExpr + "></p></td>");
                            }
                            else
                            {
                                paramHtml.Append("<td><p class='paramtd1 cap '>" + paramCaption + "</p><p class=paramtd2><input type=text id='" + parameterName + "' name='" + parameterName + "' data-caption='" + paramCaption + "' value='" + paramValue + "' class='" + onlyTime + "tem Family form-control '" + CallValidateExpr + "></p></td>");
                            }

                        }
                        else if ((paramMOE.ToLower() == "pick list"))
                        {
                            // paramHtml.Append("<td><p class=paramtd1 style='background-color: transparent;'><font class=cap>" + paramCaption + "</font></p><p class=paramtd2 style='background-color: transparent;'><input type=text id='" + parameterName + "' name='" + parameterName + "' value='" + paramValue + "'  class='tem Family'" + CallValidateExpr + " style='width:100%' >&nbsp;<a><img align='middle' src=\"../AxpImages/pickList.png\" id=\"img~" + parameterName + "\" onclick=\"javascript:ParamSearchOpen(this);\" style=\"cursor:hand;vertical-align: middle;\" border=\"0\" /></a></p></td>");
                            if (dynamicFilterString.EndsWith("|"))
                                dynamicFilterString = dynamicFilterString.Substring(0, dynamicFilterString.Length - 1);
                            //paramHtml.Append("<td><p class='paramtd1 cap '>" + paramCaption + "</p><p class='paramtd2 input-group picklist'><input type='text' id='" + parameterName + "' name='" + parameterName + "' value='" + paramValue + "'  class='tem Family inputClass2 form-control' " + CallValidateExpr + "  autocomplete='off'/><span class=\"picklisttxtclear icon-arrows-remove\" title=\"Clear Text\"></span><a class='btn btn-default input-group-addon' style='' onclick=\"javascript:ParamSearchOpen(this);\" id=\"img~" + parameterName + "\"><span class='glyphicon glyphicon-search icon-basic-magnifier'  title=\"Search\"></span></a></p></td>");
                            paramHtml.Append("<td><p class='paramtd1 cap '>" + paramCaption + "</p><div class='input-group inputClass2parent paramtd2 picklist'><div class='form-group has-feedback has-clear '><input type='text' id='" + parameterName + "' name='" + parameterName + "' data-caption='" + paramCaption + "' value='" + paramValue + "'  class='tem Family inputClass2 form-control form-control' " + CallValidateExpr + " autocomplete='off'/><span class=\"picklisttxtclear form-control-clear glyphicon glyphicon-remove form-control-feedback hidden icon-arrows-remove\" title=\"Clear Text\"></span></div><span class='input-group-btn' onclick=\"javascript:ParamSearchOpen(this);\" id=\"img~" + parameterName + "\"><button type='button' class='btn'><i class='fa fa-search'></i></button></span></div></td>");
                        ...
                        {
                            paramHtml.Append("<td><p class='paramtd1 cap '>" + paramCaption + "<span class=\"ivparamselect\"> *</span></p><p class=paramtd2><select id='" + parameterName + "' name='" + parameterName + "' data-caption='" + paramCaption + "'  class='combotem Family form-control input-sm selectPaddingFix' onmouseover=\"showHideTooltip(this.id);\"  " + CallValidateExpr + ">");
                            ...
                            for (i = 0; i <= arrFillList.Count - 1; i++)
                            {
                                ...
                                else
                                {
                                    if (i == 0)
                                    {
                                        paramHtml.Append("<option value=''>" + Constants.EMPTYOPTION + "</option>");
                            ...
                            paramHtml.Append("</select></p></td>");
                            arrFillList.Clear();
                            isSqlFld = false;
                        }
                        else if (paramMOE.ToLower() == "select" & isSqlFld == false)
                        {
                            paramHtml.Append("<td><p class='paramtd1 cap '>" + paramCaption + "<span class=\"ivparamselect\"> *</span></p><p class=paramtd2><select id='" + parameterName + "' name='" + parameterName + "' data-caption='" + paramCaption + "' class='combotem Family form-control input-sm selectPaddingFix' " + CallValidateExpr + "></select></p></td>");
                        }
                        else if (((paramMOE.ToLower() == "multi select")))
                        {
                            paramHtml.Append("<td><p  class='paramtd1 cap ' valign='top'>"
                                    + paramCaption + "</p><div class='paramtd2'>");
                            ...
                            else
                            {
                                ...
                                paramHtml.Append("<div class=\"input-group-hide multiChkSpan gridstackCalcChecklist\" ><label class=\"input-group-addon hide\" ><input type=\"checkbox\" class=\"tokenSelectAll\" onclick=\"checkAllCheckBoxTokens(this);\" />Select All</label><div class=\"autoinput-parent\"><input name=\"" + parameterName + "\" id='" + parameterName + "' data-caption=\"" + paramCaption + "\" type=\"text\" class=\"multiFldChk tem Family form-control\"" + " data-type=\"checkbox\" data-valuelist=\"" + string.Join(separator.ToString(), arrFillList.ToArray().Where(x => !string.IsNullOrEmpty(x.ToString())).ToArray()) + "\"  data-selectedlist=\"" + paramValue + "\"  data-separator=\"" + separator.ToString() + "\" />");
    ...
    private void LoadActivities()
    {
        ...
        string sqlQuery = string.Empty;
        string result = string.Empty;
        string errorLog = logobj.CreateLog("GetLoginActivity.", Session["nsessionid"].ToString(), "GetLoginAct-" + iName + string.Empty, "new");
    ...
    public static string SaveJsonInDB(string jsonString, bool isListView)
    {
        ...
        {
            logobj.CreateLog(ex.Message, sid, fileName, string.Empty);
        }
    ...
    [WebMethod]
    public static object ActionBtnClick(string iName, string ivkey, string iXml, string actName, string dcTableDellst)
    {
        var objIview = (IviewData)HttpContext.Current.Session[ivkey];
        if (objIview == null)
            return new { result = Constants.ERAUTHENTICATION };
        string filename = "iviewActionBtnClick-" + iName;
        LogFile.Log logobj = new LogFile.Log();
        //ASBExt.WebServiceExt asbExt = new ASBExt.WebServiceExt();
        string errlog = logobj.CreateLog("", HttpContext.Current.Session["nsessionid"].ToString(), filename, "");
        iXml = iXml.Replace("♦♦trace♦♦", errlog);
        if ((actName == "deletetstdef" || actName == "saveaststdef") && iName == "mainrepo")
        {
            iXml = iXml.Replace("<root ", "<root webaxpapp='" + HttpContext.Current.Session["webProj"] + "' ");
            iXml += HttpContext.Current.Session["webAxApps"].ToString();
        }
        ...
        if (ires == string.Empty)
            result = "failure";
        if (iName == "mainrepo" && ires != string.Empty && !ires.Contains("\"error\"") && (actName == "readtstructdef" || actName == "deletetstdef" || actName == "saveaststdef"))
        {
            XmlDocument xmlDoc = new XmlDocument();
            xmlDoc.LoadXml(iXml.ToString());
            string transId = xmlDoc.SelectNodes("//root/params/str")[0].InnerText;
            if (actName == "readtstructdef" || actName == "saveaststdef")
            {
                JObject objConfig = JObject.Parse(ires);
                string strXML = objConfig["message"][0]["structxml"].ToString();
                ...
                ires = objWebServiceExt.CallWriteDefinitionWS("mainpro", readIXm, strXML);

                if ((iName == "mainrepo" && HttpContext.Current.Session["AxConfigPage"] != null && HttpContext.Current.Session["AxConfigPage"].ToString().StartsWith("addform~")))
                {
                    try
                    {
                        FDW fdwObj = FDW.Instance;
                        ...
                        string keyPattern = fObj.MakeKeyName(Constants.RedisIvData, "ad___acs", HttpContext.Current.Session["username"].ToString(), "*", -1);
            ...
            else
            {
                ...
                readIXm += HttpContext.Current.Session["webAxApps"].ToString() + HttpContext.Current.Session["axApps"].ToString();
                readIXm += HttpContext.Current.Application["axProps"].ToString() + HttpContext.Current.Session["axGlobalVars"].ToString() + HttpContext.Current.Session["axUserVars"].ToString() + "</root>";
                ires = objWebServiceExt.CallDeleteDefinitionWS("mainpro", readIXm);
            }
            Util.Util objUtil = new Util.Util();
            objUtil.DeleteCachedDataKeys("tstruct", iXml);
        }
        ires = ires.Replace("'", ";quot");
        ires = ires.Replace("\\", ";bkslh");
        ires = ires.Replace("\n", "<br>");
        ...
        try
        {
            ...
            if (exportResp != null)
            {
                if (exportResp["result"][0]["error"] != null)
                {
                    ...
                    //returnString = exportResp["result"][0]["error"].ToString();
                }
                else
                {
                    ...
                    if (uploadResponse != null && uploadResponse.ToString() != returnString)
                    {
                        if (uploadResponse["result"][0]["error"] != null)
                        {
                            ...
                            //returnString = uploadResponse["result"][0]["error"].ToString();
                        }
                        else
                        {
                            ...
                            if (importResponse != null && importResponse.ToString() != returnString)
                            {
                                if (importResponse["result"][0]["error"] != null)
                                {
                                    ...
                                    //returnString = importResponse["result"][0]["error"].ToString();
                                }
                                else
                                {
                                    returnString = PublishUpdatepublish(importResponse, structs, schema, destScripts);
    ...
    private static JObject PublishExportfromsource(string structs, string schema, string destScripts, string comment, string devPublish)
    {
        ...
        try
        {
            ...
            string scriptURL = System.Configuration.ConfigurationManager.AppSettings["RestDllPath"] + "ASBPublishRest.dll/datasnap/rest/TASBPublishRest/";

            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(scriptURL + "Exportfromsource");
            request.Method = "POST";
            request.ContentType = "application/json";

            param["axpapp"] = HttpContext.Current.Session["webProj"].ToString(); /*HttpContext.Current.Session["project"].ToString();*/
            param["webaxpapp"] = HttpContext.Current.Session["project"].ToString();
    ...
    public static string DeleteRedisKeys(string xml, string schemaName)
    {
        ...
        {
            logobj.CreateLog("Iview - DeleteRedisKeys -" + ex.Message + "", HttpContext.Current.Session["nsessionid"].ToString(), "Iview - DeleteRedisKeys", string.Empty);
        }
        return "success";
    }
    ...
    public XmlDocument GetRedisData()
    {
        ...
        {
            logobj.CreateLog("Iview - GetRedisKeys -" + ex.Message + "", HttpContext.Current.Session["nsessionid"].ToString(), "Iview - GetRedisKeys", string.Empty);
        }
        return doc1;
    }
