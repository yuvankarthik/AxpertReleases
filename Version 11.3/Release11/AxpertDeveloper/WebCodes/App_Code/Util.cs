...
namespace Util
{
    ...
    public class Util
    {
        ...
        public string ParseJSonErrorNode(string result, bool throwEx = true)
        {
            ...
            for (int i = 0; i < newResult.Length; i++)
            {
                ...
                try
                {
                    ...
                    {
                        if (message["error"].Type.ToString() == "Array")
                        {
                            msg = (JArray)message["error"];
                        }
                        else if (message["error"]["msg"] != null)
                        {
                            return message["error"]["msg"].ToString();
                        }
                    }
        ...
        public TStructDefNew GetTstructDefObjNew(string errorLog, string transid)
        {
            TStructDefNew strObj = null;
            LogFile.Log logobj = new LogFile.Log();
            try
            {
                CacheManagerNew cacheMgr = new CacheManagerNew(errorLog);
                strObj = cacheMgr.GetStructDef(HttpContext.Current.Session["project"].ToString(), HttpContext.Current.Session["nsessionid"].ToString(), HttpContext.Current.Session["user"].ToString(), transid, HttpContext.Current.Session["AxRole"].ToString());
            }
            catch (Exception ex)
            {
        ...
        public string GetDefConnection(string defDbUser)
        {
            string defConName = string.Empty;
            if (defDbUser != string.Empty)
            {
                FDR fdrObj = new FDR();
                defConName = fdrObj.StringFromRedis(Constants.AXDEFAPPS_NODE, defDbUser);
                if (defConName == string.Empty)
                {
                    string ScriptsPath = HttpContext.Current.Application["ScriptsPath"].ToString();
                    FileInfo fi = new FileInfo(ScriptsPath + "\\axapps.xml");
                    if (fi.Exists)
                    {
                        ...
                        XmlNodeList pNode = doc.SelectNodes("/connections");
                        foreach (XmlNode node in pNode)
                        {
                            foreach (XmlNode chnode in node.ChildNodes)
                            {
                                string dbuser = chnode["dbuser"].InnerXml;
                                string userDtls = dbuser.Split('\\')[0];
                                if (userDtls == defDbUser)
                                {
                                    defConName = chnode.Name;
                                    FDW fdwObj = FDW.Instance;
                                    fdwObj.SaveInRedisServer(Constants.AXDEFAPPS_NODE, defConName, Constants.AXDEFAPPS_NODE, defDbUser);
                                    return defConName;
                                }
                            }
                        }
        ...
        public string GetWebAxApps(string strProj)
        {
            string connRes = string.Empty;
            ...
            if (connRes == string.Empty)
            {
                ...
                HttpContext.Current.Session["webaxdb"] = db;
                ...
                HttpContext.Current.Session["webaxApps"] = contents;
                GetWebDBConnection(strProj, contents);
            }
            return connRes;
        }
        public void GetWebDBConnection(string strProj, string contents)
        {
            ...
            HttpContext.Current.Session["webdbuser"] = dbuser;
            HttpContext.Current.Session["webaxconstr"] = cs;
            HttpContext.Current.Session["webaxdb"] = dbtype;
        ...
        public string ConvertPerfXmlToXml(string res, IviewData objIview, bool removePerfEmptyRow = false, bool isExport = false)
        {
            XmlDocument xml = new XmlDocument();
            DataSet ds = new DataSet();
            string perfXml = string.Empty;
            XmlNodeList headernodes = default(XmlNodeList);
            XmlNodeList baseDataNodes = default(XmlNodeList);
            string completeres = string.Empty;
            string result = string.Empty;
            string headchidnode = string.Empty;
            completeres = res;
            xml.LoadXml(res);

            XmlNode cNode = xml.SelectSingleNode("//comps");
            if (cNode != null)
                cNode.ParentNode.RemoveChild(cNode);

            XmlNode headrow = xml.SelectSingleNode("//headrow");
            if (headrow != null)
                headrow.ParentNode.RemoveChild(headrow);

            res = xml.InnerXml;
            //res = RemoveAppSessKeyAtt(res, "appsessionkey");
            res = res.Replace("<data perfxml=\"true\">", "");
            ...
            GetSerialNoForPerfXml(ds.Tables[0], "column1");
            ...
            for (int i = 1; i < ds.Tables[0].Rows.Count; i++)
            {
                for (int j = 0; j < ds.Tables[0].Columns.Count; j++)
                {
                    ...
                    if (ds.Tables[0].Rows[i][j].ToString() == "" && !isExport)
                        ds.Tables[0].Rows[i][j] = "";

                    int idx = objIview.ColFld.IndexOf(ds.Tables[0].Columns[j].ToString().ToLower());
                    if (idx != -1 && objIview.ColType[idx].ToString() == "d")
                    {
                        ds.Tables[0].Rows[i][j] = ConvertPerfDate(ds.Tables[0].Rows[i][j].ToString());
                    }
                }
            }
            try
            {
                if (removePerfEmptyRow && ds.Tables[0].Rows.Count > 0)
                {
            ...
            result = ds.GetXml().ToString();

            XmlDocument doc1 = new XmlDocument();
            doc1.LoadXml(completeres);

            XmlNode rNode = doc1.SelectSingleNode("//rowdata");
            rNode.ParentNode.RemoveChild(rNode);

            XmlDocument doc2 = new XmlDocument();
            doc2.LoadXml(result);
            XmlNode newNode = doc2.DocumentElement;

            XmlNode headrow1 = doc1.SelectSingleNode("//headrow");
            XmlNode importNewsItem = doc1.ImportNode(newNode, true);
            if (headrow1 != null)
                headrow1.ParentNode.AppendChild(importNewsItem);

            headernodes = doc1.SelectNodes("//headrow");
            ArrayList NodesToRemove = new ArrayList();
            foreach (XmlNode headernode in headernodes)
            {
                baseDataNodes = headernode.ChildNodes;
                int childCount = headernode.ChildNodes.Count;

                foreach (XmlNode baseDataNode in baseDataNodes)
                {
                    headchidnode = baseDataNode.Name;
                    if ((baseDataNode.Name == "rowno") || (baseDataNode.Name == "pivotghead") || (baseDataNode.Name == "axrowtype") || (baseDataNode.Name == "axp__font"))
                    {
                        NodesToRemove.Add(baseDataNode);
                    }
                }

            }

            foreach (XmlElement nodeList in NodesToRemove)
            {
                nodeList.ParentNode.RemoveChild(nodeList);
            }

            perfXml = doc1.InnerXml.ToString();
            //perfXml = RemoveAppSessKeyAtt(perfXml, "appsessionkey");
            perfXml = perfXml.Replace("<data perfxml=\"true\">", "<data>");
            perfXml = perfXml.Replace("<rowdata>", "");
            perfXml = perfXml.Replace("</rowdata>", "");
            return perfXml;
        }
        ...
        public void GetAxIntelliviewDet(string ivName)
        {
            string Ax_user = string.Empty;
            ...
            Ax_user = HttpContext.Current.Session["user"].ToString();
            if (HttpContext.Current.Session["project"] != null)
            {
                ...
                qry = "select * from AxIntelliviewDet where Ax_user = '" + Ax_user + "' and ivname = '" + ivName + "'";
                LogFile.Log logobj = new LogFile.Log();
                errorLog = logobj.CreateLog("GetChoices webservice", "1", "GetSchemas-", "new");

                inputXML = "<sqlresultset axpapp='" + proj + "' transid='' sessionid='" + HttpContext.Current.Session.SessionID + "' trace='" + errorLog + "' appsessionkey='" + HttpContext.Current.Session["AppSessionKey"].ToString() + "' username='" + HttpContext.Current.Session["username"].ToString() + "'>";
                inputXML += "<sql>" + qry + "</sql>";
                ...
                try
                {
                    ...
                    result += objWebServiceExt.CallGetChoiceWS(" ", inputXML);
                    XmlDocument doc = new XmlDocument();
                    doc.LoadXml(result);
                    XmlNode IvCredentialNode = doc.SelectSingleNode("/sqlresultset/response/row");
                    if (IvCredentialNode != null && IvCredentialNode.HasChildNodes)
                    {
                        for (int i = 0; i < IvCredentialNode.ChildNodes.Count; i++)
                        {
                            var child = IvCredentialNode.ChildNodes;
                            string nodeName = child[i].Name.ToString();

                            if (nodeName == "Username")
                            {
                                userName = child[i].InnerText;
                                HttpContext.Current.Session["iv_userName" + ivName] = userName;
                            }
                            else if (nodeName == "Password")
                            {
                                password = child[i].InnerText;
                                HttpContext.Current.Session["iv_password" + ivName] = password;
                            }
                            else if (nodeName == "Companyname")
                            {
                                companyName = child[i].InnerText;
                                HttpContext.Current.Session["iv_companyName" + ivName] = companyName;
                            }
                            else if (nodeName == "Reportname")
                            {
                                reportName = child[i].InnerText;
                                HttpContext.Current.Session["iv_reportName" + ivName] = reportName;

                            }
                            else if (nodeName == "Filtervalues")
                            {
                                filterValues = child[i].InnerText;
                                HttpContext.Current.Session["iv_filterValues" + ivName] = filterValues;
                            }
                            else if (nodeName == "Reporttype")
                            {
                                reportType = Char.Parse(child[i].InnerText.ToLower());
                                HttpContext.Current.Session["iv_reportType" + ivName] = reportType;
                            }
                        }
                    }
                }
        ...
        public bool GetAuthentication()
        {
            ...
            if (mapServer.IndexOf(":;bkslh") == -1)
            {
                ...
                if (mapUsername != string.Empty && mapPassword != string.Empty)
                {
                    try
                    {
                        using (new Impersonator(mapUsername, "agilelabs", mapPassword))
                            return true;
                    }
                    catch (Exception ex)
                    {
        ...
        public void TempAttaServerFiles()
        {
            if (HttpContext.Current.Session["attGridFileServer"] != null && HttpContext.Current.Session["attGridFileServer"].ToString() != string.Empty)
            {
                ...
                if (lstServerFiles.Length > 0 && GetAuthentication())
                {
            ...
            if (HttpContext.Current.Session["AxpAttFileServer"] != null)
            {
                ...
                if (attServerFiles.Count > 0 && GetAuthentication())
                {
        ...
        public string GetAxRelationsNew(string TransId)
        {
            string AxRelKeys = string.Empty;
            try
            {
        ...
        public bool isRealTimeCacEnabledNew(string transid, TStructDefNew tstDef)
        {
            if (tstDef == null)
            {
                tstDef = GetTstructDefObjNew("From Listview", transid);
            }
            ...
            if (fldIdx != -1)
            {
                TStructDefNew.FieldStruct fld = (TStructDefNew.FieldStruct)tstDef.flds[fldIdx];
                if (fld.expr.ToLower() == "{t}")
                    isRTCacheEnabled = true;
            }
            return isRTCacheEnabled;
        ...
        public object GetFormLoadKey(string transId)
        {
            string datakey = string.Empty, arrayStr = string.Empty, strflKeys = string.Empty, devOptionKey = string.Empty, fldarrkey = string.Empty, arrayData = string.Empty;
            try
            {
                int keyIndex = -1;
                ...
                {
                    FDR fdrObj = (FDR)HttpContext.Current.Session["FDR"];
                    fldarrkey = fdrObj.MakeKeyName(Constants.FORMLOADARRAY, transId);
                    int ExistKeyind = fdrObj.GetKeyIndex(fldarrkey);
                    if (ExistKeyind == 0)
                    {
                        strflKeys = fdrObj.ReadKey(fldarrkey);
                        if (strflKeys == string.Empty)
                        {
                            datakey = fdrObj.MakeKeyName(Constants.FORMLOADINDEX, transId, "", "", 0);
                            arrayStr = strflKeys;
                        ...
                        else
                        {
                            ...
                            if (keyIndex != -1)
                            {
                                arrayData = keyList[keyIndex];
                                datakey = fdrObj.MakeKeyName(Constants.FORMLOADINDEX, transId, "", "", keyIndex);
                                //datakey = GetRedisServerFieldkey(transId, Constants.FORMLOADINDEX, "", keyIndex);
                            }
                            else
                                datakey = fdrObj.MakeKeyName(Constants.FORMLOADINDEX, transId, "", "", strflKeys.Split('¿').Count());
                            //datakey = GetRedisServerFieldkey(transId, Constants.FORMLOADINDEX, "", strflKeys.Split('¿').Count());
                            arrayStr = strflKeys;
                            devOptionKey = cacheTime;
                        }
                    }
                    else if (ExistKeyind == -1)
                    {
                        datakey = fdrObj.MakeKeyName(Constants.FORMLOADINDEX, transId, "", "", 0);
                        arrayStr = strflKeys;
                        devOptionKey = cacheTime;
            ...
            return new { dataKey = datakey, arrKey = fldarrkey, arrStr = strflKeys, devOption = devOptionKey, arrayData = arrayData };
        }

        public string GetFormLoadData(string transId, dynamic flKey)
        {
            string result = string.Empty;
            try
            {
                FDR fdrObj = (FDR)HttpContext.Current.Session["FDR"];
                string datakey = flKey.GetType().GetProperty("dataKey").GetValue(flKey, null);
                string arrkey = flKey.GetType().GetProperty("arrKey").GetValue(flKey, null);
                string arrStr = flKey.GetType().GetProperty("arrStr").GetValue(flKey, null);
                string devOption = flKey.GetType().GetProperty("devOption").GetValue(flKey, null);
                string arrayData = flKey.GetType().GetProperty("arrayData").GetValue(flKey, null);
                if (arrStr != string.Empty && devOption == string.Empty)
                {
                    string keyDate = arrayData.Split('♦').Last();
                    string sysDate = DateTime.Now.ToString("dd/MM/yyyy");
                    if (keyDate == sysDate)
                        result = fdrObj.ReadKey(datakey);
                    //result = fObj.StringFromRedis(datakey);
                }
                else if (arrStr != string.Empty && devOption != string.Empty)
                {
                    int cacheMins = 0;
        ...
        public void SetFormLoadData(string loadRes, string transId, dynamic flKey)
        {
            try
            {
                string datakey = flKey.GetType().GetProperty("dataKey").GetValue(flKey, null);
                ...
                {
                    int ArrKeyCount = arrStr.Split('¿').Count() - 1;
                    int dataIndex = int.Parse(datakey.Split('-')[2]);
                    if (dataIndex > ArrKeyCount)
                    {
                        if (devOption != string.Empty)
                            strflKeys = arrStr + "¿" + stsGlobal + "♦" + DateTime.Now.ToString("dd/MM/yyyy HH:mm");
                        else
                            strflKeys = arrStr + "¿" + stsGlobal + "♦" + DateTime.Now.ToString("dd/MM/yyyy");
                    }
                    else
                    {
                        string replKey = string.Empty;
                        string existingKey = arrayData.Substring(0, arrayData.LastIndexOf('♦'));
                        if (devOption != string.Empty)
                            replKey = existingKey + "♦" + DateTime.Now.ToString("dd/MM/yyyy HH:mm");
                        else
                            replKey = existingKey + "♦" + DateTime.Now.ToString("dd/MM/yyyy");
                        strflKeys = arrStr.Replace(arrayData, replKey);
                    }
                }

                FDW fdwObj = FDW.Instance;
                if (fdwObj.WriteKey(arrkey, strflKeys))
                    fdwObj.WriteKey(datakey, loadRes);
            }
            catch (Exception ex)
            {
                LogFile.Log logobj = new LogFile.Log();
        ...
        public void DeleteCachedDataKeys(string keyType, string keyInfo = "")
        {
            try
            {
                ...
                if (HttpContext.Current.Session["webdbuser"] != null)
                    fdwObj.Initialize(HttpContext.Current.Session["webdbuser"].ToString());
                string schemaName = string.Empty;
                if (HttpContext.Current.Session["webdbuser"] != null)
                ...
                if (keyType == "tstruct")
                {
                    ...
                    ArrayList tstCachedKeys = fObj.GetAllKeys(tKey, schemaName);
                ...
                else if (keyType == "iview")
                {
                    ...
                    string ivKey = schemaName + "-" + IviewName + "-*";
                    ArrayList ivCachedKeys = fObj.GetAllKeys(ivKey, schemaName);
        ...
        public void GetNewGlobalVars(string tsid)
        {
            LogFile.Log logobj = new LogFile.Log();
            try
            {
                ...
                string iXml = "<root scriptpath='" + HttpContext.Current.Application["ScriptsPath"].ToString() + "' axpapp='" + HttpContext.Current.Session["webProj"] + "' trace='" + errlogapp + "' sessionid='" + HttpContext.Current.Session["nsessionid"] + "' appsessionkey='" + HttpContext.Current.Session["AppSessionKey"].ToString() + "' user='" + HttpContext.Current.Session["username"].ToString() + "'>";
                ...
                string result = objWebServiceExt.CallGetNewGlobalVarsWS(tsid, iXml);
        ...
        private void ParseGlobalVarsResult(string result)
        {
            ...
            foreach (XmlNode xmlNode in childNode.ChildNodes)
            {
                ...
                //if (xmlNode.Name.ToLower() == "project")
                //{
                //    string strProj = (xmlNode.InnerText == "" ? HttpContext.Current.Session["project"] : xmlNode.InnerText).ToString();
                //    HttpContext.Current.Session["project"] = strProj;
                //    globalVariables.Add(xmlNode.Name + "=" + strProj);
                //}
                //else
        ...
        public void GetAxARMConnection(string strProj)
        {
            ...
            if (strProj != string.Empty)
            {
                try
                {
                    ...
                    if (jsoncontents != "noarmconnection")
                    {
                        try
                        {
