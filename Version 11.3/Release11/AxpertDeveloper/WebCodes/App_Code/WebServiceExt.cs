...
namespace ASBExt
{
    ...
    public class WebServiceExt
    {
        ...
        public string CallGetParamsWS(string transId, string inputXML, IviewData objIview)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (objIview.StructureXml != string.Empty && objIview.isObjFromCache)
                    {
        ...
        public string CallGetIViewWS(string transId, string inputXML, string iSTructure, IviewData objIview)
        {
            ...
            {
                string[] redisIpPort = ConfigurationManager.AppSettings["redisIP"].ToString().Split(':');
        ...
        public string ExecuteSQL(string transid, string sqlQuery, string format = "")
        {
            ...
            if (HttpContext.Current.Session["project"] == null)
                if (format.ToLower() == "json")
                    ...
                    return "<error>" + utilObj.SESSTIMEOUT + "</error>";

            string errorLog = logobj.CreateLog("Call ExecuteSQ", HttpContext.Current.Session["nsessionid"].ToString(), "CallExecuteSQ-" + transid + "", "new");
            string inputXML = string.Empty;

            inputXML = "<sqlresultset axpapp='" + HttpContext.Current.Session["webProj"].ToString() + "' sessionid='" + HttpContext.Current.Session["nsessionid"].ToString() + "' trace='" + errorLog + "' appsessionkey='" + HttpContext.Current.Session["AppSessionKey"].ToString() + "' username='" + HttpContext.Current.Session["username"].ToString() + "' responseformat='" + format + "'  ><sql>" + sqlQuery + "</sql>";
            inputXML += HttpContext.Current.Session["webAxApps"].ToString() + HttpContext.Current.Application["axProps"].ToString() + HttpContext.Current.Session["axGlobalVars"].ToString() + HttpContext.Current.Session["axUserVars"].ToString() + "</sqlresultset>";
        ...
        public string ExecuteSQLFromDef(string transid, string sqlQuery, string format = "")
        {
            string result = string.Empty;
            if (HttpContext.Current.Session["project"] == null)
                if (format.ToLower() == "json")
        ...
        public string saveToolbar(string inputXML)
        {
            ...
            try
            {
                ...
                result = GetAppSessionKey("CallSaveToolBar", result);
            }
            catch (Exception ex)
            {
                CallExceptionErrorPage("CallSaveToolBar", ex.Message.ToString(), "CallSaveToolBar");
            }
            return result;
        ...
        public string CallWriteDefinitionWS(string transId, string inputXML, string structure)
        {
            string result = string.Empty;

            try
            {
                result = asbAction.WriteDefinition(inputXML, structure);
                result = GetAppSessionKey("CallWriteDefinitionWS", result);
            }
            catch (Exception ex)
            {
                CallExceptionErrorPage("WriteDefinition", ex.Message.ToString(), "WriteDefinition-" + transId + "");
            }
            return result;
        }

        public string CallDeleteDefinitionWS(string transId, string inputXML)
        {
            string result = string.Empty;

            try
            {
                result = asbAction.DeleteDefinition(inputXML);
                result = GetAppSessionKey("CallDeleteDefinitionWS", result);
            }
            catch (Exception ex)
            {
                CallExceptionErrorPage("DeleteDefinition", ex.Message.ToString(), "DeleteDefinition-" + transId + "");
            }
            return result;
        }

        public string CallGetMETAdataWS(string inputXML)
        {
            string result = string.Empty;

            try
            {
                result = asbAction.GetMETAdata(inputXML);
                result = GetAppSessionKey("CallGetMETAdataWS", result);
            }
            catch (Exception ex)
            {
                CallExceptionErrorPage("GetMETAdata", ex.Message.ToString(), "GetMETAdata");
            }
            return result;
        }

        public string CallUpdateMETAdataWS(string inputXML, string structure)
        {
            string result = string.Empty;

            try
            {
                result = asbAction.UpdateMETAdata(inputXML, structure);
                result = GetAppSessionKey("CallUpdateMETAdataWS", result);
            }
            catch (Exception ex)
            {
                CallExceptionErrorPage("UpdateMETAdata", ex.Message.ToString(), "UpdateMETAdata");
            }
            return result;
        }

        public string CallCopyTableDataToAppDBWS(string transId, string inputXML)
        {
            string result = string.Empty;

            try
            {
                result = asbAction.CopyTableDataToAppDB(inputXML);
                result = GetAppSessionKey("CallCopyTableDataToAppDBWS", result);
            }
            catch (Exception ex)
            {
                CallExceptionErrorPage("CopyTableDataToAppDB", ex.Message.ToString(), "CopyTableDataToAppDB-" + transId + "");
            }
            return result;
        }
    }
