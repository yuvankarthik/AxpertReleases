...
$(document).ready(function (event) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (match) {
                ...
                fancytreeObj.$container.addClass("h-400px overflow-auto border-0");
        ...
        error: function (error) {
            showAlertDialog("error", "Error while getting Responsibility data.");
            callParentNew("closeFrame();", "function");
        }
    });
    ...
    function getUrlParams() {
        ...
        if (action == "add") {
            ...
            resblk.classList.remove("invisible");
        }
    }
    ...
    $("#save_btn").off("click").on("click", function () {
        ...
        if (responsibilityName != "" && xml != "") {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.d == "<success>success</success>")
                        showAlertDialog("success", "Responsibilities saved successfully!");
                    else
                        showAlertDialog("error", "Responsibilities not saved, please try again later.");
                    callParentNew("closeFrame();", "function");
                },
                error: function (error) {
                    showAlertDialog("error", "Responsibilities not saved, please try again later.");
            ...
            showAlertDialog("warning", "Responsibility name should not empty.");
            callParentNew("closeFrame();", "function");
        } else if (xml == "") {
            showAlertDialog("warning", "Please select Responsibility!");
            callParentNew("closeFrame();", "function");
        }
    });
});
