...
function addRowFormGrid(dcNo, newRowNo, calledFrom) {
    ...
    if ($(`#gridAddBtn${dcNo}`)?.[0] != undefined) {
        gridHtml += "<div class=\"addFormGridbutton d-inline-flex me-2\"><button id=\"gridAddBtnAdd" + dcNo + "\" onclick=\"editTheRow(''," + dcNo + ",'" + strRowNo + "',event)\" class=\"btn btn-primary btn-sm shadow-sm\" type=\"button\" title=\"Add\">Add Row</button></div>" + btnRemove + "<div class=\"clearfix\"></div></div>";
    }
...
function DeleteFromGridRow(dcNo, rowFrmNo, elem) {
    ...
    if ($(`#colScroll${dcNo} [id^=gridrowWrap${dcNo}-]`).length == 1) {//if (delRowNo == "001") {
        clearFormGridFirstRow(dcNo, delRowNo);
    } else {
...
function clearFormGridFirstRow(dcNo, delRowNo) {
    if ($(`#gridrowWrap${dcNo}-${delRowNo}`)) {
        DeleteGridRow(dcNo, delRowNo + "F" + dcNo, undefined);

        var myInterval = setInterval(() => {
            if (isExcelImpDelWS == 'false') {
