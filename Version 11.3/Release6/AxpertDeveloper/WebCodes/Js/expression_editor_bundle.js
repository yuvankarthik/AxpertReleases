...
if (typeof CodeMirror !== "undefined" && !CodeMirror.showHint) {
    (function(mod) {
        ...
        function Widget(completion, data) {
            ...
            let _thisId = cm.getTextArea().id;
            if (_thisId == 'exp_editor_fcscript000F1') {
                const element = document.querySelectorAll('ul.CodeMirror-hints')[0];
            ...
            for (var i = 0; i < completions.length; ++i) {
                ...
                if (_thisId == 'exp_editor_fcscript000F1') {
                    if (typeof expression_FormControl_config.function[getText(cur)] != "undefined") {
            ...
            if (widget.data.list[0].text) {
                if (cm.getTextArea().id == 'exp_editor_fcscript000F1' && typeof expression_FormControl_config.function[widget.data.list[0].text]!="undefined") {
                    hints.getElementsByTagName('li').length == 0 ? hints.style.display = "none" : '';
                    curCMEditorHints.html(expression_FormControl_config.function[widget.data.list[0].text].selectText + "<br>" + expression_FormControl_config.function[widget.data.list[0].text].example);
                }
                else if (typeof expression_config.function[widget.data.list[0].text] != "undefined")
            ...
            cm.addKeyMap(this.keyMap = buildKeyMap(completion, {
                moveFocus: function(n, avoidWrap) {                    
                    ...
                    if (widget.data.list[0].text) {
                        if (cm.getTextArea().id == 'exp_editor_fcscript000F1')
                            curCMEditorHints.html(expression_FormControl_config.function[widget.data.list[widget.selectedHint].text].selectText + "<br>" + expression_FormControl_config.function[widget.data.list[widget.selectedHint].text].example);
                        else
            ...
            var CodeMirrorParser = Class(Parser, {
                ...
                autocomplete: function(cm, options, CodeMirror) {
                        ...
                        if (dynamic || grammar_tokens) {
                            ...
                            if (cm.getTextArea().id == 'exp_editor_fcscript000F1' && list.length > 0) {
                                for (let i = list.length - 1; i >= 0; i--) {
                                    if (typeof expression_FormControl_config.function[list[i].text] == "undefined") {
                                        list.splice(i, 1);
                                    }
                                }
                            }
    ...
    (function() {
        ...
        const createExpressionEditor = function({textarea,customValidationFn}) {
            if (typeof textarea === "string") {
                ...
                $(textarea).parent().append('<span class="expHints hide"></span>');
            }
            ...
            textarea.nextElementSibling.classList.add(..."w-100 border".split(" "));
