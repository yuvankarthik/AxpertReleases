...
#region TstructDefNew Class
/// <summary>
/// TStructDefNew class contains complete TStruct related Data. This also has method to 
...
public class TStructDefNew
{
    ...
    public DesignObjectNew axdesignJObjectNew = new DesignObjectNew();
    ...
    public TStructDefNew(string xml, DataTable customDS, DataSet dsRapidDef)
    {
    ...
    private void XmlPageLoadDWBButton(XmlDocument FormBtnNodes, int frameNo)
    {
        ...
        if (formbtnlst.Count > 0)
        {
            ...
            if (axdesignJObjectNew.dcLayout != null && axdesignJObjectNew.dcLayout != "" && axdesignJObjectNew.dcLayout != "default")
            {
            ...
            foreach (XmlNode formbtns in formbtnlst)
            {
                foreach (XmlNode btnNodes in formbtns)
                {
                    ...
                    if ((btnNodes.Attributes["cat"] != null) && (btnNodes.Attributes["cat"].Value == "btn"))
                    {
                        ...
                        if (axdesignJObjectNew.buttonFieldFont != null && axdesignJObjectNew.buttonFieldFont.Count > 0 && btnId != "")
                        {
                            var frbtnfld = axdesignJObjectNew.buttonFieldFont.Where(elm => elm.id == btnId).ToList();
    ...
    private void FillNGSqlDepParents()
    {
        ...
        for (int dcIdx = 0; dcIdx < dcs.Count; dcIdx++)
        {
            ...
            {
                TStructDefNew.FieldStruct fld = (TStructDefNew.FieldStruct)flds[i];
                fIdx = i;
    ...
    public string GetPagePositionHtml(string dcNo)
    {
        ...
        else
        {
            ...
            {
                TStructDataNew tstData = new TStructDataNew("", transId, "0", this);
    ...
    public string GetJScriptArrays(TStructDefNew strObj)
    {
    ...
    /// <param name="strObj"></param>
    /// <returns></returns>
    public string GenerateTabScript(TStructDefNew strObj)
    {
    ...
    private string GetFieldHtml(int dcNo)
    {
        ...
        if (!isGrid && (axdesignJObjectNew.dcLayout != null && axdesignJObjectNew.dcLayout != "" && axdesignJObjectNew.dcLayout != "default"))
        {
            fcwidth = int.Parse(axdesignJObjectNew.fieldCaptionWidth);
            fcwidth = fcwidth / 10;
        ...
        for (int i = 0; i <= flds.Count - 1; i++)
        {
            ...
            if (fld.fldframeno == dcNo)
            {
                ...
                if (!fld.visibility)
                {
                    ...
                    if (!isGrid)
                    {
                        if (axdesignJObjectNew.buttonFieldFont != null && axdesignJObjectNew.buttonFieldFont.Count > 0)
                        {
                            var frbtnfld = axdesignJObjectNew.buttonFieldFont.Where(elm => elm.id == name).ToList();
                            if (frbtnfld != null && frbtnfld.Count > 0)
                            {
    ...
    private string GetRadioGroupHTML(FieldStruct fld, bool isGrid, string name, int fldNo, string[] fldTlhw, string width, string[] groupValues)
    {
        ...
        string hiddenName = fld.name + "\" +nrno+\"";
        string spnclass = (fld.type.ToUpper() == "V") ? "radio rdinput radiovert" : "radio rdinput radiohori";
        string bgColor = "";
        if (fld.fldColor != string.Empty)
            bgColor = " background-color:" + GetFieldColor(fld.fldColor) + "; ";
        //changes for lable colors
    ...
    private string GetImageHTML(FieldStruct Fld, bool IsGrid, string Name, int fldNo, string[] fldTlhw, string width, int fcwidth)
    {
        ...
        else
        {
            ...
            imgHTML.Append("<div class=\"input-group input-group-sm\"><div class=\"image-input image-input-outline\" data-kt-image-input=\"true\" style=\"background-image: url(/assets/media/avatars/blank.png)\">");
            imgHTML.Append("<div class=\"shadow-sm w-250px h-150px imageFileUpload d-flex\"><span class=\"material-icons material-icons-style material-icons-4x d-flex m-auto align-self-center\">file_upload</span></div>");
            imgHTML.Append("<img id=\"" + Name + "\" class=\"profile-pic image-input-wrapper w-250px h-150px d-none\" src=\"\">");
    ...
    private string GetCustLabelHTML(string fldName, int fldDcNo)
    {
        ...
        try
        {
            if (axdesignJObjectNew.formLabel != null && axdesignJObjectNew.formLabel.Count > 0)
            {
                string colFldWidth = "", colFldGridStackItem = "", colDivInputPadding = "";
                if (axdesignJObjectNew.dcLayout != null && axdesignJObjectNew.dcLayout != "" && axdesignJObjectNew.dcLayout != "default")
                {
                ...
                var frLbl = axdesignJObjectNew.formLabel.Where(elm => elm.dc == fldDcNo & elm.afterField == (fldName == "" ? "divDc" + fldDcNo : fldName)).ToList();
    ...
    public string GetTabFullHTML(int dcNo, int rowCnt, TStructDataNew tstData, string isTab, string IsTabDis)
    {
    ...
    public string GetTabDcHTML(int dcNo, TStructDataNew tstDataObj, string IsTabDisabled)
    {
    ...
    public int GetRowCount(int dcNo, TStructDataNew tstData)
    {
        int rCnt = 0;
    ...
    /// <seealso cref="GetFinalHtml"/>
    /// <returns> returns the field html. </returns>   
    private string GetTabFieldHtml(int dcNo, TStructDataNew tstData, string IsTabDisabled, string rowNo, string keyColValue, ArrayList keyCols, ArrayList keyRowNos)
    {
        //NOTE: Assigning javascript functions for grid and non-grid fields's is done seperately,
        //as the grid field javascript functions are written into the hidden field through the variable gridHiddenHtml. 
        //The difference for grid functions is usage of \"+quo+\" instead of \".   
    ...
    /// <seealso cref="GetFinalHtml"/>
    /// <returns> returns the field html. </returns>
    private string GetFillGridDataFieldHtml(int dcNo, TStructDataNew tstData, string IsTabDisabled, string rowNo, string keyColValue, ArrayList keyCols, ArrayList keyRowNos)
    {
        //NOTE: Assigning javascript functions for grid and non-grid fields's is done seperately,
        //as the grid field javascript functions are written into the hidden field through the variable gridHiddenHtml. 
        //The difference for grid functions is usage of \"+quo+\" instead of \".   
    ...
    public ArrayList GetGroups(int dcNo, TStructDataNew tstData)
    {
        DcStruct dc = (DcStruct)(dcs[dcNo - 1]);
    ...
    /// <returns></returns>
    public string GetDefaultFormatGridHtml(int dcNo, TStructDataNew tstData, int rowCnt, string isTab, string isTabDis)
    {
        DcStruct dc = (DcStruct)(dcs[dcNo - 1]);
    ...
    /// </summary>
    /// <returns></returns>
    public string GetFormatHtmlForGroups(string selectedGroups, TStructDataNew tstData, string isTab)
    {
        string dcData = string.Empty;
        string dcHtml = string.Empty;
        ArrayList groups = new ArrayList();
    ...
    /// <returns></returns>
    public string ExecuteSql(string sql, TStructDataNew tstData, int dcNo)
    {
        string result = string.Empty;
    ...
    /// <param name="IsTabDis"></param>
    /// <returns></returns>
    public string GetFormatDcHtml(int dcNo, int rowCnt, TStructDataNew tstData, string isTab, string IsTabDis)
    {
        DcStruct dc = (DcStruct)(dcs[dcNo - 1]);
    ...
    /// <param name="tstData"></param>
    /// <returns></returns>
    public string GetGroupRowHtml(int dcNo, ArrayList groupValues, string rowNo, TStructDataNew tstData)
    {
        string rowHtml = string.Empty;
    ...
    /// <param name="rowNo"></param>
    /// <param name="tstData"></param>
    private void UpdateSubTotalValues(DcStruct dc, string rowNo, TStructDataNew tstData)
    {
    ...
    protected string xywhGridStack(FieldStruct fld = new FieldStruct(), bool isTable = false, int GridX = 0, int GridY = 0)
    {
        ...
        if (axdesignJObjectNew == null)
        {
            axdesignJObjectNew = new DesignObjectNew();
            axdesignJObjectNew.newDesign = true;
        }
        //try
        //{
        //    isFreshDesign = axdesignJObjectNew.transid != null && axdesignJObjectNew.transid == transId ? false : true;
        //}
        //catch (Exception ex)
        //{
        //    isFreshDesign = true;
        ...
        DcNew curDc;
        string DcLayout = axdesignJObjectNew.dcLayout;
        if (DcLayout == null || DcLayout == "default")
        {
            if (axdesignJObjectNew.dcs == null)
            {
                axdesignJObjectNew.dcs = new List<DcNew>();
            }
            curDc = axdesignJObjectNew.dcs.FirstOrDefault(elm => elm.dc_id == dcNo.ToString());
        }
        else
        {
            if (axdesignJObjectNew.newdcs == null)
            {
                axdesignJObjectNew.newdcs = new List<DcNew>();
            }
            curDc = axdesignJObjectNew.newdcs.FirstOrDefault(elm => elm.dc_id == dcNo.ToString());
        }
        try
        {
            if (curDc == null)
        ...
        {
            //while (axdesignJObjectNew.dcs.Count != dcNo)
            //{
            //    axdesignJObjectNew.dcs.Add(new Dc());
            //    if (axdesignJObjectNew.dcs.Count == dcNo)
            //    {
            //        axdesignJObjectNew.dcs[axdesignJObjectNew.dcs.Count - 1].dc_id = dcNo.ToString();
            //    }
            //}
        }
        ...
        if (!isTable)
        {
            ...
            else
            {
                ...
                if (fDsign == null)
                {
                    ...
                    if (!fld.visibility)
                    {
                        ...
                        {
                            curDc.fieldsDesign = new List<FieldsDesignNew>();
                        }

                        if (fld.datatype == "special" && fDsign == null && !axdesignJObjectNew.newDesign && !isFreshDesign)
                        {
                            freshAutoArrange = true;
                        }
                        ...
                        curDc.fieldsDesign.Add(new FieldsDesignNew());
                        int newX = 0;
                        int newY = 0;
                        ...
                        //if (fld.ctype == "button" && fDsign == null && !axdesignJObjectNew.newDesign && !isFreshDesign)
                        if (fld.ctype == "button" && fDsign == null && !axdesignJObjectNew.newDesign)
                        {
            ...
            if (DcLayout == null || DcLayout == "default")
            {
                if (axdesignJObjectNew.dcs.FirstOrDefault(elm => elm.dc_id == dcNo.ToString()) == null)
                {
                    axdesignJObjectNew.dcs.Add(curDc);
                }
                else
                {
                    int index = axdesignJObjectNew.dcs.IndexOf(axdesignJObjectNew.dcs.FirstOrDefault(elm => elm.dc_id == dcNo.ToString()));
                    axdesignJObjectNew.dcs[index] = curDc;
                }
            }
            else
            {
                if (axdesignJObjectNew.newdcs.FirstOrDefault(elm => elm.dc_id == dcNo.ToString()) == null)
                {
                    axdesignJObjectNew.newdcs.Add(curDc);
                }
                else
                {
                    int index = axdesignJObjectNew.newdcs.IndexOf(axdesignJObjectNew.newdcs.FirstOrDefault(elm => elm.dc_id == dcNo.ToString()));
                    axdesignJObjectNew.newdcs[index] = curDc;
                }
            }
        ...
        else
        {
            ...
            try
            {
                ...
                if (curDc.tableDesign == null)
                {
                    curDc.tableDesign = new List<TableDesignNew>();
                }

                if (fld.datatype == "special" && tDsign == null && !axdesignJObjectNew.newDesign && !isFreshDesign)
                {
                    freshAutoArrange = true;
                }
                ...
                curDc.tableDesign.Add(new TableDesignNew());
                ...
                else
                {
                    if (!fld.visibility)
                    {
                        ...
                        if (fld.ctype == "button" && tDsign == null && !axdesignJObjectNew.newDesign)
                        {
    ...
    public DesignObjectNew getDesignObjectFromJson(string axdesignJson)
    {
        DesignObjectNew axdesignJObjectNew = new DesignObjectNew();
        try
        {

            axdesignJObjectNew = JsonConvert.DeserializeObject<DesignObjectNew>(axdesignJson.TrimStart(new char[] { '[' }).TrimEnd(new char[] { ']' }));
            //if (axdesignJObjectNew.transid != transId)
            //{
            //    axdesignJObjectNew = null;
            //}
            if (axdesignJObjectNew.transid == null)
            {
                axdesignJObjectNew.newDesign = true;
                isFreshDesign = true;
            }
    ...
    private void getDesignDataPerf()
    {
        ...
        axdesignJObjectNew = getDesignObjectFromJson(axdesignJson);
        if (axdesignJObjectNew == null)
        {
            axdesignJObjectNew = new DesignObjectNew();
            axdesignJObjectNew.newDesign = true;
...
public class DcNew
{
    ...
    public List<FieldsDesignNew> fieldsDesign { get; set; }
    public List<TableDesignNew> tableDesign { get; set; }
}
[Serializable()]
public class DesignObjectNew
{
    ...
    public List<FormLabelNew> formLabel { get; set; }
    public List<ButtonFieldFontNew> buttonFieldFont { get; set; }
    public List<DcNew> dcs { get; set; }
    public List<DcNew> newdcs { get; set; }
}
[Serializable()]
public class FormLabelNew
{
...
[Serializable()]
public class ButtonFieldFontNew
{
    public string ftype { get; set; }
    public string id { get; set; }
    public string fontFamilly { get; set; }
