...
function createAutoComplete(fld) {
    ...
    autoComFlds.autocomplete({
                    ...
                    if ($j.inArray(fieldName, sourceAcMetaJsonFlds) > -1) {
                        ...
                        $.ajax({
                            ...
                            data: JSON.stringify({
                                ...
                                pickArrow: pickarrow, parentsFlds: parentFldVal, rfSave: isrefreshsave
                            }),
                            dataType: 'json',
                            contentType: "application/json",
                            success: function (data) {
        ...
        open: function (event, ui) {
            ...
            $("#" + preasentAutoId).append("<li class='autoComFooter' style='float: left;z-index:999999;width:" + widthOfUl + " ;position: relative;top: " + topOfUl + ";left:" + leftOfUl + ";'>" + $("#autoadv" + fldNameAc).html() + "</li>");
            if (!$(".dvheightframe").hasScrollBar() && isPopupAutoComp) {
            ...
            countPerPage = rcount;
            if (isMobile)
                $("#" + preasentAutoId).css({ "width": fldwidth + "px" });
            if (AutPageNo == 1) {
                if (PageCount == 1) {
                    $("#" + preasentAutoId + " #autoleft" + fldNameAc).prop("disabled", true).addClass("disabled");
                    $("#" + preasentAutoId + " #autoright" + fldNameAc).prop("disabled", true).addClass("disabled");
                    ...
                    if (listDivTabOrder.length > 0) {
                        ...
                        $.each(listDivTabOrder, function (i, indTabOr) {
                            if (curFldTabOrder < indTabOr) {
                                ...
                                if (typeof $("#" + $(NextFldId).attr("id")).attr("disabled") == "undefined" && typeof $("#" + $(NextFldId).attr("id")).attr("readonly") == "undefined" && $("#" + $(NextFldId).attr("id")).parents(".grid-stack-item").is(':visible')) {
                                    if (isAutoComSelWithTab == false && AxOldValue != presAutoFldElem.val()) {
